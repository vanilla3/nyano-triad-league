...
- Follow-up (2026-02-20, step7): normalized Replay clipboard toast copy in `apps/web/src/features/match/replayUiActions.ts` to remove mojibake artifacts (`Copied` / `Copy failed`) and aligned `apps/web/src/features/match/__tests__/replayUiActions.test.ts` assertions.
- Follow-up verification (2026-02-20, step7): `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- replayUiActions useMatchShareClipboardActions useMatchReplayActions replayActionRunners webShare`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build` all passed (sandboxed vitest hit spawn EPERM once; rerun elevated passed).
- Follow-up (2026-02-20, step8): aligned share button labels with native-share-first behavior by changing `Copy share URL` wording to `Share URL` in `MatchShareActionsRow`, `MatchGuestPostGamePanel`, and Replay share entry points.
- Follow-up verification (2026-02-20, step8): `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- MatchShareActionsRow MatchGuestPostGamePanel replayUiActions useMatchReplayActions`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build` all passed (sandboxed vitest hit spawn EPERM once; rerun elevated passed).
- Follow-up (2026-02-20, step9): improved template sharing quality by fixing `buildMatchShareTemplateMessage` mojibake to `Nyano Triad Replay\n{url}` and enabling native-share-first fallback for `copyShareTemplate` in `useMatchReplayActions` (clipboard fallback retained).
