...
- [x] 置いた/奪った/連鎖/勝利の瞬間が「気持ちいい」方向へ増強される
- [x] ただし常時派手にならない（演出の出る頻度・面積が適切）
- [x] `prefers-reduced-motion` と `data-vfx=off` で演出が抑制される
- [x] レイアウトが崩れない（CLS無し。DOM追加で押し広げない）
- [x] Match の処理・勝敗・ターン進行に影響しない
