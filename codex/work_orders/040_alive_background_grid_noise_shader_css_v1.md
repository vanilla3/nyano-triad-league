...
- [x] 背景が “生きている” と感じるが、主役（カード/盤面/HUD/ボタン）が負けない
- [x] 既存背景のアニメーションと競合して破綻しない（ちらつき、モアレ、読みにくさが増えない）
- [x] `prefers-reduced-motion` と `data-vfx=off` で停止し、静的でも成立する
- [x] パフォーマンスが落ちない（特にモバイル幅 390px）
