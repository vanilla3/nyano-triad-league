...
/// @notice Core match resolver v2 (ETH-only, deterministic).
/// @dev Supports the "Core + Tactics" subset plus ONE Synergy step:
///      - Shadow ignores warning mark debuff (-1)
...
library TriadEngineV2 {
    ...
    function resolve(INyanoPeace nyano, TranscriptV1.Data calldata t) internal view returns (Result memory r) {
        ...
        for (uint8 turn = 0; turn < 9; turn++) {
            ...
            } else if (mark != NONE_U8 && cell == mark && !_isShadow(nyano, tokenId)) {
                eUp = _dec1(eUp);
                eRight = _dec1(eRight);
                eLeft = _dec1(eLeft);
                eDown = _dec1(eDown);
    ...
    function _isShadow(INyanoPeace nyano, uint256 tokenId) private view returns (bool) {
        (uint8 classId, , uint8 rarity) = nyano.getTrait(tokenId);
        // Trait derivation (nyanoTrait_v1): rarity=3 uses classTrait; classId=5 => shadow.
        return rarity == 3 && classId == 5;
    }
