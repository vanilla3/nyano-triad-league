...
- [x] 2026-02-18 i18n/e2e follow-up: Match ? Decks ????????????????????????????apps/web/e2e/guest-game.spec.ts ???UI??????pnpm -C apps/web test / typecheck / build / pnpm.cmd -C apps/web e2e:ux / ??3spec ????
...
- [x] 2026-02-19 WO001 status sync: `codex/work_orders/001_fix_nft_images.md` を現状実装に合わせて更新（shared resolver + DOM/Pixi fallback + retry/preload + tests）。
- [x] 2026-02-19 WO002 status sync: `codex/work_orders/002_fix_replay_share.md` を現状実装に合わせて更新（BASE_URL 対応 share URL + Replay共有の安定化 + replay bundle versioning + tests）。
...
- [x] 2026-02-19 WO003-B layer split: `PixiBattleRenderer` の board レイヤー（shadow/base/detail）をトークン化し、品質 tier による切替を `boardLayerTokensForQuality`（`apps/web/src/engine/renderers/pixi/boardLayerTokens.ts`）へ集約。cell shadow も同トークン参照に統一。
...
- [x] 2026-02-20 WO037-step119: Match/Replay の Stage VFX state 初期化と変更ハンドリングを `useStageVfxPreference` に統一し、ページ側の重複を削減。
...
- [x] 2026-02-20 WO037-step120: Stage VFX セレクトの option label 生成を `resolveStageVfxOptionLabel` に共通化し、Match/Replay の JSX 重複を削減。
...
- [x] 2026-02-20 WO046-step6: Match setup share URL now uses native-share-first fallback (`apps/web/src/features/match/useMatchShareClipboardActions.ts`) with URL compatibility preserved and clipboard fallback retained; verified by `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- useMatchShareClipboardActions useMatchReplayActions replayActionRunners webShare`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build` (sandbox vitest EPERM once; elevated rerun passed).
- [x] 2026-02-20 WO046-step7: Replay copy-toast copy in `apps/web/src/features/match/replayUiActions.ts` was normalized (`Copied` / `Copy failed`) and `apps/web/src/features/match/__tests__/replayUiActions.test.ts` was synchronized; verified by `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- replayUiActions useMatchShareClipboardActions useMatchReplayActions replayActionRunners webShare`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build` (sandbox vitest EPERM once; elevated rerun passed).
- [x] 2026-02-20 WO046-step8: share button wording now matches native-share-first behavior (`Share URL`) across `MatchShareActionsRow`, `MatchGuestPostGamePanel`, and Replay share controls; verified by `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- MatchShareActionsRow MatchGuestPostGamePanel replayUiActions useMatchReplayActions`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build` (sandbox vitest EPERM once; elevated rerun passed).
- [x] 2026-02-20 WO046-step9: share template quality was hardened by replacing mojibake template text with `Nyano Triad Replay\n{url}` and enabling native-share-first fallback for `copyShareTemplate` in `useMatchReplayActions`; verified by `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- matchShareLinks useMatchReplayActions webShare MatchGuestPostGamePanel`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build` (sandbox vitest EPERM once; elevated rerun passed).
- [x] 2026-02-20 WO044-step2: `scripts/check_text_hygiene.mjs` was hardened with expanded mojibake signatures (including the newly observed share-template corruption pattern), and verification passed with `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- matchShareLinks useMatchReplayActions webShare MatchGuestPostGamePanel`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build` (sandbox vitest EPERM once; elevated rerun passed).
- [x] 2026-02-20 WO006-008 follow-up: DuelStageMint unit test expectations were updated to current layer structure and full apps/web verification (typecheck/lint/test/build) was completed.
- [x] 2026-02-20 WO043-followup-2: stabilized stage-focus announcer stack visibility by adding `min-h-[1px]` to `stage-focus-announcer-stack` in `apps/web/src/features/match/matchStageLayoutClasses.ts` so empty state no longer collapses and Playwright visibility assertion remains stable.
- [x] 2026-02-20 WO043-followup-2 verification: `pnpm.cmd -C apps/web e2e -- e2e/stage-focus.spec.ts` (15 passed), `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web test` (219 files / 1763 tests) all passed.
- [x] 2026-02-20 WO014-doc-sync: aligned `codex/execplans/014_uiux_polish_qa_shareworthy_v8.md` with actual completion state (WO-044/045/046 checked, decision log/outcome added) and fixed formatting glitch in `codex/work_orders/044_match_copy_mojibake_cleanup_text_hygiene_v1.md`.
- [x] 2026-02-20 WO043-followup-3: added regression assertion in `apps/web/src/features/match/__tests__/matchStageLayoutClasses.test.ts` to require `min-h-[1px]` on `stage-focus-announcer-stack`, guarding against future zero-height collapse regressions.
- [x] 2026-02-20 WO043-followup-3 verification: `pnpm.cmd -C apps/web test -- matchStageLayoutClasses` (spawn EPERM in sandbox once; rerun with elevated permission passed) and `pnpm.cmd -C apps/web lint` passed.
- [x] 2026-02-20 WO006/043-followup-4: implemented missing `mint-hit` / `mint-pressable` / `mint-ui-pressable*` CSS foundations and added motion tokens in `apps/web/src/mint-theme/mint-theme.css` so press feedback and 44px hit targets are consistently enforced across Match/Replay/Home mint controls.
- [x] 2026-02-20 WO006/043-followup-4 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web test -- MintPressable MintRulesetPicker MatchShareActionsRow MatchGuestPostGamePanel MintTabNav` (spawn EPERM in sandbox once; rerun with elevated permission passed), `pnpm.cmd -C apps/web build`, and `pnpm.cmd -C apps/web e2e -- e2e/stage-focus.spec.ts` (15 passed) all passed.
- [x] 2026-02-20 WO006-followup-5: added `apps/web/src/components/mint/__tests__/MintPressable.test.tsx` to lock class composition and disabled link fallback behavior of `MintPressable` (`button`/`link`/`aria-disabled` branches).
- [x] 2026-02-20 WO006-followup-5 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web test -- MintPressable` (spawn EPERM in sandbox once; rerun with elevated permission passed), and `pnpm.cmd -C apps/web build` all passed.
- [x] 2026-02-20 WO007-followup-6: added missing announcer-slot layout CSS in `apps/web/src/mint-theme/mint-theme.css` (`mint-announcer-stack`, `mint-status-summary*`, `mint-nyano-reaction-slot*`) to reserve cut-in/status regions and reduce layout jitter while keeping stage-focus mobile commit actions visible.
- [x] 2026-02-20 WO007-followup-6 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web test -- NyanoReactionSlot` (spawn EPERM in sandbox once; rerun with elevated permission passed), `pnpm.cmd -C apps/web e2e -- e2e/stage-focus.spec.ts` (15 passed after mobile-height tuning), and `pnpm.cmd -C apps/web build` all passed.
- [x] 2026-02-20 WO006-followup-7: normalized remaining key UI transitions in `apps/web/src/styles.css` (`nav-item`, `replay-progress__bar`, `replay-highlight-marker`) to shared transition tokens (`--transition-fast` / `--transition-normal`) instead of hard-coded millisecond values.
- [x] 2026-02-20 WO006-followup-7 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, and `pnpm.cmd -C apps/web build` all passed.
- [x] 2026-02-20 WO008-followup-8: replaced Home onboarding placeholder status copy (`TODO` / `DONE` / `AUTO`) with production-facing Japanese labels (`未完了` / `完了` / `自動`) in `apps/web/src/pages/Home.tsx`.
- [x] 2026-02-20 WO008-followup-8 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, and `pnpm.cmd -C apps/web build` all passed.
- [x] 2026-02-20 WO039-followup-9: normalized the remaining hard-coded transition timings in `apps/web/src/mint-theme/mint-theme.css` (`mint-nyano-reaction`, `mint-drawer-backdrop`, `mint-drawer`, `mint-battle-hud__progress-fill`, `mint-duel-card`) to shared motion tokens (`--mint-motion-duration-pop`, `--mint-motion-ease-standard`, `--mint-motion-ease-pop`).
- [x] 2026-02-20 WO039-followup-9 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, and `pnpm.cmd -C apps/web build` all passed.
- [x] 2026-02-20 WO039-followup-10: expanded motion-token transition unification into `apps/web/src/rpg-theme/rpg-theme.css` and removed remaining hard-coded inline transition strings in `apps/web/src/components/GameResultOverlayMint.tsx` / `apps/web/src/components/NyanoReaction.tsx` by switching to `--transition-*` tokens.
- [x] 2026-02-20 WO039-followup-10 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- NyanoReactionSlot NyanoReaction.timing` (sandbox spawn EPERM once; elevated rerun passed), `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, and `pnpm.cmd -C apps/web build` all passed.
- [x] 2026-02-20 WO039-followup-11: added `apps/web/src/components/__tests__/motionTransitionTokenGuard.test.ts` to guard against hard-coded transition durations in `mint-theme.css`, `rpg-theme.css`, `GameResultOverlayMint.tsx`, and `NyanoReaction.tsx`.
- [x] 2026-02-20 WO039-followup-11 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- motionTransitionTokenGuard` (sandbox spawn EPERM once; elevated rerun passed), `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, and `pnpm.cmd -C apps/web build` all passed.
- [x] 2026-02-20 WO039/041/046-followup-12: implemented perceived-responsiveness polish by adding 0ms press ACK feedback (`data-press-ack` in `MintPressable` + immediate press styles), tightening Nyano cut-in emphasis (smaller footprint/shorter timing + high-impact-only burst), restricting stage impact burst to meaningful high-impact moments, introducing mint Z-depth tokens for overlay/cutin layers, and adding share-ready cues in Match share rows and Mint result overlay.
- [x] 2026-02-20 WO039/041/046-followup-12 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web test -- MintPressable NyanoReaction.timing matchStageReaction useReplayStageImpactBurst MatchShareActionsRow MatchGuestPostGamePanel` (sandbox spawn EPERM once; elevated rerun passed), `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build`, `pnpm.cmd -C apps/web test --`, and `pnpm.cmd -C apps/web e2e -- e2e/stage-focus.spec.ts` all passed.
- [x] 2026-02-20 WO043-followup-13: Stage-focus toolbar UX baseline was unified in `apps/web/src/mint-theme/mint-theme.css` by tokenizing control hit size (`--mint-focus-toolbar-hit-size`), adding immediate press feedback for `.stage-focus-toolbar .btn`, enabling `touch-action: manipulation`, and scaling targets for coarse pointers without route/protocol changes.
- [x] 2026-02-20 WO043-followup-13 verification: `pnpm.cmd lint:text`, `pnpm.cmd -C apps/web lint`, `pnpm.cmd -C apps/web typecheck`, `pnpm.cmd -C apps/web build`, and `pnpm.cmd -C apps/web e2e -- e2e/stage-focus.spec.ts` (15 passed) all passed.
- [x] 2026-05-01 Vercel production recovery: root `vercel.json` now explicitly builds the monorepo Vite app with `pnpm build:web` and serves `apps/web/dist`, keeping SPA route rewrites intact.
- [x] 2026-05-01 Vercel production verification: deployed production prebuilt output to `https://v0-nyano-triad-league.vercel.app`; verified `/`, `/match`, `/replay`, and `/battle-stage` return 200.
- [x] 2026-05-01 Release-check unblock: fixed the escaped quote in `DuelStageMint.test.tsx` JSX fixture that was preventing `apps/web typecheck` from parsing the test.
- [x] 2026-05-01 Release-check unblock: moved `/motions` hooks before the production guard return and guarded the VFX side effect with `devOk`, preserving the dev-only gate while satisfying hook-order lint.

- [x] 2026-06-12 全コードベース監査: 技術的負債の棚卸しと優先度付けを `docs/99_dev/TECH_DEBT_AUDIT_2026-06-12_ja.md` に記録。
- [x] 2026-06-12 コミュニティパック: LICENSE(MIT・種別は要オーナー確認) / CONTRIBUTING.md（日英） / CODE_OF_CONDUCT.md を新設、README を刷新（公開URL・NFT不要プレイ導線・プロトコル入口）。
- [x] 2026-06-12 図鑑ビジュアル超強化: 未定義だった `.mint-card-browser` CSS を完全実装。エッジ合計ベースの見た目ティア（S/A/B リング+バッジ、`cardTier.ts`）、ホバーシャイン、ステッガー入場。`/nyano` をオフライン全カード図鑑化（クリックで詳細プレビュー）。
- [x] 2026-06-12 対戦結果演出: 勝利時の紙吹雪・光線・スコアカウントアップ（reduced-motion / data-vfx 準拠）+ X ポストボタン（`shareToX`）。
- [x] 2026-06-12 テスト負債解消: 日本語化 WIP に未追従だった unit 36 件 / e2e 10 ファイルを期待値整合、旧文字化け入りビジュアルベースライン更新、vitest testTimeout 30s 化。
- [x] 2026-06-12 退行復旧: Rulesets.tsx（おすすめルールセット機能消失）をステージ版から復元、BoardViewMint セルのキーボード操作（tabindex/Enter/Space + mint-pressable）を復元。
- [x] 2026-06-12 衛生: デッドCSS 約2,600行削除、eslint `no-explicit-any` warn 化、root `pnpm test` に web 追加、handoff zip 除外、`lint:text` の `_archive` 除外 + 破損 WO007 退避。
- [ ] 次: 初回 git コミットの作成（現状 master に履歴ゼロ＝事故で全損リスク。旧 stash の棚卸しも）。
