...
- [x] 2026-02-21 WO010-followup-2: added a stable Playwright major-flow guardrail in `apps/web/e2e/home-arena-match-replay-flow.spec.ts` that verifies Home -> Arena -> Match navigation, keeps pre-result share/replay controls gated on Match, and validates Replay final-step loading via deterministic `z` transcript payload.
- [x] 2026-02-21 WO010-followup-2 verification: `pnpm.cmd -C apps/web e2e -- e2e/home-arena-match-replay-flow.spec.ts`, `pnpm.cmd -C apps/web lint`, and `pnpm.cmd lint:text` passed.
