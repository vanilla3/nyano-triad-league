...
- `apps/web/src/pages/Stream.tsx`
  ...
  - Localized summary labels (`Overlay` -> `オーバーレイ`, `Match bus` -> `対戦連携`).
...
- `apps/web/src/pages/Replay.tsx`
  ...
  - Result banner helper labels were polished (`step` -> `手順`, `matchIdをコピー`, `対局ログをコピー (Transcript)`).
- `apps/web/src/pages/Rulesets.tsx`
  - Classic section heading/subcopy was changed to Japanese-first wording (`クラシック（ベータ）Classic`).

### Verify
- `pnpm.cmd test` (apps/web) OK
- `apps/web/e2e/rulesets-ux-guardrails.spec.ts`
  - Updated heading assertion to JP/EN bilingual match (`Ruleset Registry|ルールセット一覧`) for copy-change resilience.
...
- `apps/web/src/pages/Replay.tsx`
  ...
  - Refined overlay sync panel copy (`手順 (step)` / `オーバーレイへ送信（手順同期）`).
  - Refined detail panel labels (`タイブレーク`, JSON copy button labels, transcript/result section labels).
- `apps/web/src/pages/Rulesets.tsx`
  - Search placeholder and row meta labels adjusted to Japanese-first wording (`名前 / rulesetId / キー`, `エンジン`, `キー未対応`).
  - Updated unmapped action hint (`対戦リンク未対応`).
- `apps/web/e2e/rulesets-ux-guardrails.spec.ts`
...
- `pnpm.cmd test` (apps/web) OK
- `apps/web/src/pages/Replay.tsx`
  - Stage focus transport toggle labels changed to Japanese-first (`操作を隠す/表示`) with English compatibility tokens retained in parentheses.
...
### What
- `apps/web/src/pages/Events.tsx`
  - Replaced user-facing `�b��` wording with `�Q�l�l` in season-point summary/ranking sections.
...
- `apps/web/src/pages/Stream.tsx`
  - Local fallback error messages were updated to Japanese-first wording:
    - `RPC�ڑ��G���[ (RPC connection failed)`
    - `�O���A�g�G���[ (External integration error)`
- `apps/web/src/pages/Overlay.tsx`
