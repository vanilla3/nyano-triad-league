...
> 1ã‚³ãƒŸãƒƒãƒE1ã¾ã¨ã¾ã‚Šã§è¿½è¨˜ã™ã‚‹ï¼Ehy/What/VerifyE‰ã€E
## 2026-02-13 â€EWO005-B follow-up: responsive stage secondary controls

### Why
- Stage focusä¸­ã«ã‚¦ã‚£ãƒ³ãƒ‰ã‚¦å¹EŒå¤‰ã‚ã£ãŸå ´åˆã€è£œåŠ©ã‚³ãƒ³ãƒˆãƒ­ãƒ¼ãƒ«è¡¨ç¤ºãŒåEæœŸå€¤ã®ã¾ã¾ã§ã€ç‹­å¹E§æƒE ±éå¤EåºE¹E§æ“ä½œå°ç·šä¸è¶³ã«ãªã‚‹ã“ã¨ãŒã‚ã£ãŸã€E- Stageã®ã€Eç”»é¢å°ç·šã€ã‚’ç¶­æŒã™ã‚‹ãŸã‚ã€è¡¨ç¤ºé–¾å€¤ã®å…±é€šåŒ–ã¨ resizeè¿½å¾“ãŒå¿E¦ã ã£ãŸã€E
### What
- `apps/web/src/lib/stage_layout.ts`
  - `shouldShowStageSecondaryControls` ã‚’è¿½åŠ ã—ã€è£œåŠ©ã‚³ãƒ³ãƒˆãƒ­ãƒ¼ãƒ«è¡¨ç¤ºã®ãƒ–ãƒ¬ãƒ¼ã‚¯ãƒã‚¤ãƒ³ãƒˆåˆ¤å®šã‚’å…±é€šåŒ–ã€E- `apps/web/src/lib/__tests__/stage_layout.test.ts`
  - è£œåŠ©ã‚³ãƒ³ãƒˆãƒ­ãƒ¼ãƒ«è¡¨ç¤ºåˆ¤å®šï¼E90/768/769/NaNE‰ã‚’æ¤œè¨¼ã€E- `apps/web/src/pages/Match.tsx`
  - Stage controls ã®åˆæœŸè¡¨ç¤º/å†è¨ˆç®—ã‚’å…±é€šåˆ¤å®šã«åˆE›¿ã€E  - resizeæ™‚ãEè‡ªå‹•è¿½å¾“ã‚’è¿½åŠ Eˆæ‰‹å‹•ãƒˆã‚°ãƒ«å¾ŒãE manual override ã§ãƒ¦ãƒ¼ã‚¶ãƒ¼è¨­å®šã‚’å„ªå…ˆï¼‰ã€E- `apps/web/src/pages/Replay.tsx`
  - Stage transport controls ã«åŒç­‰ãE resizeè¿½å¾E+ manual override ã‚’è¿½åŠ ã€E- `apps/web/e2e/stage-focus.spec.ts`
  - mobile `replay-stage` ã§ controls ãŒåEæœŸéè¡¨ç¤ºã§ã‚ã‚‹ã“ã¨ã€E  - `Show controls` ã§å¾©å¸°ã§ãã‚‹ã“ã¨ã‚’è¿½åŠ æ¤œè¨¼ã€E  - 375px å¹E`battle-stage` ã§ Commit ãƒœã‚¿ãƒ³ãEviewport å†E«åã¾ã‚‹ã“ã¨ã‚’æ¤œè¨¼ã€E  - 375px å¹E§æ¨ªæ–¹å‘ã‚ªãƒ¼ãƒãEãƒ•ãƒ­ãƒ¼ãŒç™ºç”Ÿã—ãªãE“ã¨ã‚’æ¤œè¨¼ã€E  - game index / RPC å¤±æ•—æ™‚ã§ã‚E`replay-stage` ã® `Load replay` / `Retry load` / `Clear share params` ãŒè¡¨ç¤ºã•ã‚Œã€ãƒªã‚«ãƒãƒªå°ç·šãŒç¶­æŒã•ã‚Œã‚‹ã“ã¨ã‚’æ¤œè¨¼ã€E- `apps/web/src/lib/ai/turn_timing.ts`
  - AIè‡ªå‹•æ‰“ã¡ã®å¾E©Ÿæ™‚é–“ã‚’å†èª¿æ•´EEase/turn-step/difficulty/jitter ã‚’å¼•ãä¸Šã’E‰ã€E  - ã€Œæ—©ã™ãã¦æ©Ÿæ¢°çšE€ã«è¦‹ãˆã‚‹ãƒ†ãƒ³ãƒã‚’æŠ‘ãˆã€æ€è€E¼”åEã®ä½“æ„Ÿã‚’æ”¹å–E€E- `apps/web/src/lib/ai/__tests__/turn_timing.test.ts`
  - baseline ã¨ upper bound ã®æœŸå¾E€¤ã‚’æ›´æ–°ã—ã€èª¿æ•´å¾ŒãEæ±ºå®šè«–ã‚’æ¤œè¨¼ã€E- `apps/web/src/components/NyanoReaction.tsx`
  - `reduced-motion` ã¨ `data-vfx`EEff/low/medium/highE‰ã«å¿œã˜ã¦ cut-in timing ã‚’åEæ›¿ã€E  - `vfx=off` / reduced-motion æ™‚ãE burst ç„¡åŠ¹ + è¡¨ç¤ºæ™‚é–“çŸ­ç¸®ã§è² è·ã¨éæ¼”åEã‚’æŠ‘åˆ¶ã€E  - `vfx=low` æ™‚ãE impact ã‚’æŠ‘ãˆã¤ã¤ burst ã‚’ç„¡åŠ¹åŒ–ã€E- `apps/web/src/components/__tests__/NyanoReaction.timing.test.ts`
  - reduced-motion / vfx off / vfx low / vfx high ã® timing åˆE²ã‚’æ¤œè¨¼ã€E- `apps/web/src/lib/demo_decks.ts`
  - `buildEmergencyGuestFallbackData` ã‚’è¿½åŠ ã—ã€indexéä¾å­˜ã§ guest 5v5 ã‚’ç”Ÿæˆå¯èƒ½åŒ–ã€E- `apps/web/src/pages/Match.tsx`
  - Game Index èª­è¾¼å¤±æ•—æ™‚ã€guest mode ã§ã¯ç·Šæ€¥ãƒ•ã‚©ãƒ¼ãƒ«ãƒãƒƒã‚¯ã‚’é©ç”¨ã—ã¦å¯¾æˆ¦ç¶™ç¶šã€E  - `error/status` ã¨ toast ã§ãƒ•ã‚©ãƒ¼ãƒ«ãƒãƒƒã‚¯çŠ¶æ…‹ã‚’æ˜ç¤ºã€E- `apps/web/src/lib/__tests__/demo_decks.test.ts`
  - ç·Šæ€¥ãƒ•ã‚©ãƒ¼ãƒ«ãƒãƒƒã‚¯ãƒEƒƒã‚­ã®æ§‹æEEEv5/10æEmapE‰ã‚’æ¤œè¨¼ã€E- `apps/web/e2e/stage-focus.spec.ts`
  - battle-stage guest ã§ index èª­è¾¼å¤±æ•—æ™‚ã«ãƒ•ã‚©ãƒ¼ãƒ«ãƒãƒƒã‚¯ã§ç¶™ç¶šã§ãã‚‹ã“ã¨ã‚’æ¤œè¨¼ã€E
### Verify
...
### Why
- `/battle-stage` `/replay-stage` ã®ã‚¯ã‚¨ãƒªæ­£è¦åŒ–ãƒ­ã‚¸ãƒE‚¯ãŒãEãƒ¼ã‚¸ã”ã¨ã«é‡è¤E—ã¦ãŠã‚Šã€å›å¸°æ™‚ã«å·®åˆE‚’è¦‹è½ã¨ã—ã‚„ã™ã‹ã£ãŸã€E- Stageå°‚ç”¨ãƒ«ãƒ¼ãƒˆãEã‚¹ãƒ¢ãƒ¼ã‚¯ãEE2E ã§æœªã‚«ãƒãEã ã£ãŸãŸã‚ã€URLäº’æ›ã¨èµ·å‹•å®‰å®šæ€§ã‚’èEå‹•ã§æ‹E¿ã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/lib/stage_focus_params.ts` ã‚’è¿½åŠ :
  - `ui=engine` å¼·åˆ¶ã€`focus=1` æ­£è¦åŒ–ã€legacy `layout` ã®é™¤å»ã‚’åEé€šåŒ–ã€E- `apps/web/src/pages/BattleStage.tsx` / `apps/web/src/pages/ReplayStage.tsx`:
  - é‡è¤E—ã¦ãEŸ `useEffect` å†EEã‚¯ã‚¨ãƒªè£œæ­£å‡¦çE‚’ `normalizeStageFocusParams` ã«çµ±ä¸€ã€E- `apps/web/src/lib/__tests__/stage_focus_params.test.ts` ã‚’è¿½åŠ :
  - æ¬ æè£œå®Œã€legacy `layout=focus` å¸åã€`focus=focus` æ­£è¦åŒ–ã€no-op ã‚±ãƒ¼ã‚¹ã‚’æ¤œè¨¼ã€E- `apps/web/e2e/stage-focus.spec.ts` ã‚’è¿½åŠ :
  - `/battle-stage` `/replay-stage` ã®URLæ­£è¦åŒ–ã¨ã€ä¸»è¦UIEEand Dock / replay focus guardE‰è¡¨ç¤ºã‚’ç¢ºèªã€E
### Verify
- `pnpm -C apps/web lint`
- `pnpm -C apps/web test`
- `pnpm -C apps/web typecheck`
...
### Why
- `/battle-stage` ã¨ `/replay-stage` ã® Pixi ç›¤é¢ã‚µã‚¤ã‚ºãŒå›ºå®šå¯E‚Šã§ã€PCç’°å¢E«ã‚ˆã£ã¦ã¯ä¸»è¦æ“ä½œå°ç·šãŒç¸¦æ–¹å‘ã«ä¼¸ã³ã‚E™ã‹ã£ãŸã€E- AGENTS/Work Orderæ›´æ–°ã«åˆã‚ã›ã€Stage-firstã§ UI/UX åŸºç›¤Eˆæƒ…å ±éšå±¤ + 1ç”»é¢å°ç·šï¼‰ã‚’å…ˆã«å®‰å®šåŒ–ã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `codex/execplans/005_uiux_foundation.md` ã‚’æ–°è¦ä½œæEã—ã€Milestone A ã®å®Ÿè£E¨ˆç”»ã‚’æ–‡æ›¸åŒ–ã€E- `apps/web/src/lib/stage_layout.ts` ã‚’è¿½åŠ :
  - viewport ã¨ stageç¨®åˆ¥EEattle/replayE‰ã‹ã‚E`maxWidthPx` / `minHeightPx` ã‚’ç®—åEã€E- `apps/web/src/lib/__tests__/stage_layout.test.ts` ã‚’è¿½åŠ :
  - desktop/mobile/invalidå…¥åŠ›ãEå¢E•Œã‚’æ¤œè¨¼ã€E- `apps/web/src/pages/Match.tsx`:
  - battle-stage ã§ viewportè¿½å¾“ã‚µã‚¤ã‚ºã‚’ä½¿ç”¨ã€E  - stageå°‚ç”¨ãƒ¬ã‚¤ã‚¢ã‚¦ãƒˆã‚¯ãƒ©ã‚¹EEoot/toolbar/arena/board/cutin/dockE‰ã‚’é©ç”¨ã€E  - stage routeã§ã¯ desktop quick-commit é‡è¤E¡¨ç¤ºã‚’æŠ‘æ­¢ã€E- `apps/web/src/pages/Replay.tsx`:
  - replay-stage ã§ viewportè¿½å¾“ã‚µã‚¤ã‚ºã‚’ä½¿ç”¨ã€E  - stageå°‚ç”¨ãƒ¬ã‚¤ã‚¢ã‚¦ãƒˆã‚¯ãƒ©ã‚¹EEoot/toolbar/cutin/arena-innerE‰ã‚’é©ç”¨ã€E- `apps/web/src/styles.css`:
  - stage shell/panel ã®ãƒˆãEã‚¯ãƒ³å¤‰æ•°ã‚’è¿½åŠ ã€E- `apps/web/src/mint-theme/mint-theme.css`:
...
### Why
- åˆæœŸZIPã®æ§‹æEä¸Šã€`nyano-triad-league-starter/` ãŒåŒæ¢±ã•ã‚Œã¦ãŠã‚Šã€ãƒ¯ãƒ¼ã‚¯ã‚¹ãƒšãEã‚¹ã®ä¸­å¿EŒæ›–æ˜§ã ã£ãŸã€E- å…¬å¼æˆ¦Eˆæ¤œè¨¼å¯èƒ½E‰ã«å¿E¦ãª **matchIdã®å®šç¾©** ã‚’ã€JSONç­‰ãEæºã‚Œã‚‹å½¢å¼ã§ã¯ãªãESolidity äº’æ›ã®å›ºå®šã‚¨ãƒ³ã‚³ãƒ¼ãƒ‰ã«å¯E›ãŸã‹ã£ãŸã€E- Design v2.0 ã® Layer2EEACTICSE‰ãEæ ¸ã§ã‚ã‚‹ **è­¦æˆ’ãEãƒ¼ã‚¯** ã¯ã€æ—©æœŸã«å…¥ã‚Œã‚‹ã“ã¨ã§ã‚²ãƒ¼ãƒ ã®â€œèª­ã¿åˆã„â€ãŒç«‹ã¡ä¸ŠãŒã‚‹ã€E
### What
- `packages/triad-engine` ã‚’æ­£è¦ä½ç½®ã¸ç§»è¨­ã—ã€starteråŒæ¢±ã‚’è§£æ¶ˆã€E- Transcript v1 ã® matchId ã‚E`keccak256(abi.encode(...))` ç›¸å½“ãE **å›ºå®šABIã‚¨ãƒ³ã‚³ãƒ¼ãƒE* ã«å¤‰æ›´EESå‚çEå®Ÿè£E¼‰ã€E- Layer2Ešè­¦æˆ’ãEãƒ¼ã‚¯ã‚’å®Ÿè£E¼ˆæœ€å¤§3å›ï¼Eã‚¿ãƒ¼ãƒ³æœ‰åŠ¹Eè¸ã‚“ã ã‚«ãƒ¼ãƒ‰Triad-1E‰ã€E- ã‚´ãƒ¼ãƒ«ãƒEƒ³ãƒE‚¹ãƒˆè¿½åŠ Eˆè­¦æˆ’ãEãƒ¼ã‚¯ã®æœ‰ç„¡ã§ä¸­å¿EŒãƒ•ãƒªãƒEEã™ã‚‹/ã—ãªãE¼‰ã€E- `TRANSCRIPT_SPEC` ã«å›ºå®šABIã‚¨ãƒ³ã‚³ãƒ¼ãƒ‰ã‚’æ˜è¨˜ã€E- CIEšlockfileæœªã‚³ãƒŸãƒƒãƒˆæ®µéšã‚’æƒ³å®šã— `--frozen-lockfile` ã‚’ä¸€æ™‚è§£é™¤ã€E
### Verify
- `pnpm -C packages/triad-engine test`
- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md` ã¨ `docs/02_protocol/Nyano_Triad_League_TRANSCRIPT_SPEC_v1_ja.md` ã®æ›´æ–°ç¢ºèªE

## 2026-02-01 â€Ecommit-0003

### Why
- Design v2.0 ã® Layer2EEACTICSE‰ãEã‚‚ã†ä¸€ã¤ã®æŸ±ã§ã‚ã‚‹ **ã‚³ãƒ³ãƒœãEãƒ¼ãƒŠã‚¹** ã‚’æ—©æœŸã«å…¥ã‚Œã€E€£é–ï¼ˆã‚³ãƒ³ãƒœï¼‰ã‚’ã€Œç‹™ãE†ç”±ã€ã‚’ä½œã‚ŠãŸã‹ã£ãŸã€E- å…¬å¼æˆ¦Eˆæ¤œè¨¼å¯èƒ½E‰ã§ã¯ã€åŒã˜ãƒˆãƒ©ãƒ³ã‚¹ã‚¯ãƒªãƒ—ãƒˆã‹ã‚‰ **åŒã˜æ´¾ç”ŸåŠ¹æœï¼ˆæ¬¡ã‚¿ãƒ¼ãƒ³ãƒãƒ•EE* ãŒåEç¾ã§ãã‚‹å¿E¦ãŒã‚ã‚‹ãŸã‚ã€ã‚³ãƒ³ãƒœæ•°ã®å®šç¾©ã‚’ä»•æ§˜ã¨ã—ã¦å›ºå®šã—ãŸã‹ã£ãŸã€E- ã€Œé‹å–¶ãŒã„ãªãã¦ã‚‚ç››ã‚Šä¸ŠãŒã‚‹ã€æ–¹å‘ã«å‘ã‘ã€ERC-6551EEBAE‰ã¨ã‚¹ãƒEEã‚­ãƒ³ã‚°ã‚E**ãƒ—ãƒ­ãƒˆã‚³ãƒ«éƒ¨å“E* ã¨ã—ã¦ã©ãE½¿ãE‹ã‚’ä¸¦è¡Œã—ã¦æ•´çE—ã¦ãŠããŸã‹ã£ãŸã€E
### What
- TSã‚¨ãƒ³ã‚¸ãƒ³ã« **ã‚³ãƒ³ãƒœãEãƒ¼ãƒŠã‚¹** ã‚’å®Ÿè£E¼E  - `comboCount = 1EˆéEç½®EE+ flipCountEˆã“ã®ã‚¿ãƒ¼ãƒ³ã§ã²ã£ãã‚Šè¿”ã—ãŸæšæ•°E‰`
  - 3: MomentumEˆæ¬¡ã®è‡ªåˆEEã‚«ãƒ¼ãƒEå…¨è¾º+1EE  - 4: DominationEˆæ¬¡ã®è‡ªåˆEEã‚«ãƒ¼ãƒEå…¨è¾º+2EE  - 5+: Nyano FeverEˆæ¬¡ã®è‡ªåˆEEã‚«ãƒ¼ãƒ‰ãŒè­¦æˆ’ãEãƒ¼ã‚¯ã‚’ç„¡åŠ¹åŒ–ï¼E- å‚çEå®Ÿè£EEå‡ºåŠ›ã« `turns: TurnSummary[]` ã‚’è¿½åŠ ã—ã€UI/è§£æãŒ â€œé‹å–¶ãªã—â€Eã§ã‚‚ä½œã‚Šã‚E™ãE½¢ã«ã—ãŸã€E- ã‚´ãƒ¼ãƒ«ãƒEƒ³ãƒE‚¹ãƒˆè¿½åŠ EšMomentum ãŒæ¬¡ã‚¿ãƒ¼ãƒ³ã®ã‚«ãƒ¼ãƒ‰ã« +1 ã¨ã—ã¦åæ˜ ã•ã‚Œã‚‹ã‚±ãƒ¼ã‚¹ã€E- `RULESET_CONFIG_SPEC` ã¨ `TRANSCRIPT_SPEC` ã«ã€ã‚³ãƒ³ãƒœæ•°ã®å®šç¾©ã¨æ´¾ç”Ÿå€¤ã®æ‰±ãE‚’è¿½è¨˜ã€E- è‡ªå¾‹åŒ–æ¤œè¨ã¨ã—ã¦ `ERC6551_and_Staking_Notes_v1_ja.md` ã‚’è¿½åŠ EEBA/ã‚¹ãƒEEã‚­ãƒ³ã‚°ã®ä½¿ãE‰€ã¨æ®µéšå°åEæ¡ˆï¼‰ã€E
### Verify
- `pnpm -C packages/triad-engine test`
- ä»•æ§˜æ›´æ–°Eš`docs/02_protocol/*` ã¨ `docs/99_dev/*` ã®å·®åˆE¢ºèªE

## 2026-02-01 â€Ecommit-0004

### Why
- Layer2Eˆè­¦æˆ’ãEãƒ¼ã‚¯/ã‚³ãƒ³ãƒœãEãƒ¼ãƒŠã‚¹/å¾Œæ”»è£œæ­£E‰ãEã€Œã‚·ãƒ¼ã‚ºãƒ³ã‚Eƒ«ãƒ¼ãƒ«ã‚»ãƒEƒˆã€ã§ ON/OFF ã‚’åEã‚Šæ›¿ãˆã‚‰ã‚Œã‚‹å¿E¦ãŒã‚ã‚‹Eˆé‹å–¶ãŒæ¶ˆãˆã¦ã‚‚ã‚³ãƒŸãƒ¥ãƒ‹ãƒ†ã‚£ãŒç’°å¢E‚’ä½œã‚Œã‚‹ãŸã‚E¼‰ã€E- è¨­è¨ˆãƒ‰ã‚­ãƒ¥ãƒ¡ãƒ³ãƒEv2.0 ã«ã‚ã‚‹ã€ŒåEæ”»ãƒ»å¾Œæ”»ãƒãƒ©ãƒ³ã‚¹Eˆå¾Œæ”»åˆæ‰‹+1 ã‚‚ã—ããEå¾Œæ”»è­¦æˆE1å›ï¼‰ã€ã‚’ã€ã‚¨ãƒ³ã‚¸ãƒ³å´ã§å®‰åEã«é¸æŠã§ãã‚‹å½¢ã«ã—ãŸã‹ã£ãŸã€E- è­¦æˆ’ãEãƒ¼ã‚¯ã® Triad ä¸‹é™EE or 1E‰ãŒæ›–æ˜§ã ã¨ã€å¢E•Œã‚±ãƒ¼ã‚¹ã®çµæœãŒã‚ºãƒ¬ã¦å¾Œã‹ã‚‰åœ°çE«ãªã‚‹ãŸã‚ã€v1ã®æ±ºå®šã‚’å›ºå®šã—ãŸã‹ã£ãŸã€E
### What
- `RulesetConfigV1`EEngine-side subsetE‰ã‚’å°åEã—ã€`simulateMatchV1(..., ruleset)` ã§ãƒ«ãƒ¼ãƒ«ã‚’æŒ‡å®šå¯èƒ½ã«ã—ãŸEˆæœªæŒE®šãE `DEFAULT_RULESET_CONFIG_V1`E‰ã€E- è­¦æˆ’ãEãƒ¼ã‚¯EE  - rulesetã§ `enabled` ã‚’åEã‚Šæ›¿ãˆå¯èƒ½Eˆç„¡åŠ¹æ™‚ãE transcript ãƒ•ã‚£ãƒ¼ãƒ«ãƒ‰ã‚’ç„¡è¦–ï¼‰ã€E  - ä½¿ç”¨å›æ•°ã‚E`maxUsesPerPlayer` ã«æ˜ç¢ºåŒ–ã—ã€å¾Œæ”»ã« `secondPlayerExtraUses` ã‚’ä»˜ä¸å¯èƒ½ã«ã—ãŸã€E  - Triadä¸‹é™ã¯ **0EE..10ã«ã‚¯ãƒ©ãƒ³ãƒ—ï¼E* ã‚Ev1ã®æ±ºå®šã¨ã—ã¦ types/spec ã«æ˜è¨˜ã€E- ã‚³ãƒ³ãƒœãEãƒ¼ãƒŠã‚¹EE  - rulesetã§ `enabled` ã‚’åEã‚Šæ›¿ãˆå¯èƒ½ã«ã—ã€E–¾å€¤/åŠ¹æœé‡ã‚‚è¨­å®šã§å¤‰æ›´ã§ãã‚‹ã‚ˆã†ã«ã—ãŸEE2ãƒEƒ•ã‚©ãƒ«ãƒˆãEç¶­æŒE¼‰ã€E- å¾Œæ”»è£œæ­£EE  - rulesetã§ `secondPlayerBalance.firstMoveTriadPlus` ã‚’æŒ‡å®šã™ã‚‹ã¨ã€å¾Œæ”»ã®åˆæ‰‹ã«å…¨è¾º+Xã‚’ä»˜ä¸ã§ãã‚‹ã€E- ãƒE‚¹ãƒˆè¿½åŠ EE  - å¾Œæ”»åˆæ‰‹+1 ã®æœ‰ç„¡ã§ãƒ•ãƒªãƒEEçµæœãŒå¤‰ã‚ã‚‹ã‚±ãƒ¼ã‚¹ã€E  - å¾Œæ”»ã ã‘è­¦æˆ’ãEãƒ¼ã‚¯ +1 å›ã‚’è¨±å¯ã™ã‚‹ã‚±ãƒ¼ã‚¹EEå›ç›®ã§throwã—ãªãE¼‰ã€E
### Verify
- `pnpm -C packages/triad-engine test`
- ãƒ‰ã‚­ãƒ¥ãƒ¡ãƒ³ãƒˆæ›´æ–°Eš`docs/02_protocol/Nyano_Triad_League_RULESET_CONFIG_SPEC_v1_ja.md` / `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md` ã®å·®åˆE¢ºèªE
---

## Commit0005 â€ELayer3EEynergy / TraitåŠ¹æEv1EE
- å®Ÿè£E¼š`packages/triad-engine` ã« TraitEffectsConfig ã‚’è¿½åŠ ã—ã€v1ã®TraitåŠ¹æœã‚’æ±ºå®šè«–ã§å®Ÿè£E€E- è¿½åŠ /æ›´æ–°ã—ãŸä»•æ§˜ï¼E  - `docs/02_protocol/Nyano_Triad_League_RULESET_CONFIG_SPEC_v1_ja.md`EES shape ã«åˆã‚ã›ã¦å…·ä½“åŒ–EE  - `docs/02_protocol/Nyano_Triad_League_TRANSCRIPT_SPEC_v1_ja.md`EEarthé¸æŠãEå¿E ˆæ¡ä»¶ã‚’æEç¢ºåŒ–ï¼E  - `docs/02_protocol/Nyano_Triad_League_TRAIT_EFFECTS_SPEC_v1_ja.md`Eˆæ–°è¦ï¼šTraitã®å³å¯E»•æ§˜ï¼E
### å®Ÿè£E—ãŸTraitEE1EE- CosmicEšè§’éEç½® allTriad +1
- LightEšéš£æ¥å‘³æ–¹ allTriad +1Eˆéã‚¹ã‚¿ãƒE‚¯æ—¢å®šï¼E- ShadowEšè­¦æˆ’ãEãƒ¼ã‚¯ debuff ç„¡åŠ¹åŒ–ï¼ˆæ¶ˆè²»ã¯ã™ã‚‹EE- ForestEšæœ€åˆãEãƒ•ãƒªãƒEEè©¦è¡Œã‚’1å›ç„¡åŠ¹åŒ–ï¼EhieldEE- MetalEšé€£é–æ”»æ’E§ã¯ãƒ•ãƒªãƒEEä¸å¯
- FlameEšTriadåŒå€¤æ™‚ã€ã˜ã‚E‚“ã‘ã‚“ã§å¸¸ã«å‹ã¤Eˆç›¸æ‰‹ãŒFlameã§ãªãE ´åˆï¼E- AquaEšæ–œã‚Eæ–¹å‘ã«ã‚‚æ”»æ’E¼ˆæ–œã‚å¼·åº¦ã¯ `min(edgeA, edgeB)` æ—¢å®šï¼E- ThunderEšéš£æ¥æ•µã‚«ãƒ¼ãƒ‰ãEå…¨è¾º -1Eˆæ°¸ç¶šã€captureå‰ã«é©ç”¨EE- WindEšåEæ”»/å¾Œæ”»é¸æŠï¼Eranscriptã®firstPlayerã§è¡¨ç¾EE- EarthEšè¾ºé¸æŠE+2 / å¯¾è¾º -1EEearthBoostEdge`ã€requireChoiceæ—¢å®EtrueEE
### ã‚´ãƒ¼ãƒ«ãƒEƒ³ãƒE‚¹ãƒˆè¿½åŠ 
- Shadow ãŒè­¦æˆ’ãEãƒ¼ã‚¯ã‚’ç„¡è¦–ã™ã‚‹ã‚±ãƒ¼ã‚¹
- Forest shield ãEå›ã ã‘ãƒ•ãƒªãƒEEã‚’ç„¡åŠ¹åŒ–ã™ã‚‹ã‚±ãƒ¼ã‚¹
- Earth ã®é¸æŠã§çµæœãŒå¤‰ã‚ã‚‹ã‚±ãƒ¼ã‚¹
- Thunder ã®æ°¸ç¶šãƒ‡ãƒãƒ•
- Light ã®éš£æ¥ãƒãƒ•ã§çµæœãŒå¤‰ã‚ã‚‹ã‚±ãƒ¼ã‚¹

### æ¬¡ã®ç„¦ç‚¹
- Nyano Peace ã®ã‚ªãƒ³ãƒã‚§ãƒ¼ãƒ³å±æ€§ â†ETraitType å°åEã®æš«å®šãƒ«ãƒ¼ãƒ«EESONå…¬é–‹ï¼‹è­°è«–å¯èƒ½ãªå½¢EE- Formation bonusesEEayer3æ‹¡å¼µEE

## 2026-02-02 â€Ecommit-0006

### Why
- Layer3EEraitåŠ¹æœï¼‰ã‚’å®Ÿè£E—ãŸæ™‚ç‚¹ã§ã€æ¬¡ã®ãƒœãƒˆãƒ«ãƒãƒƒã‚¯ã¯ã€ŒNyano Peace ã®ã‚ªãƒ³ãƒã‚§ãƒ¼ãƒ³ TraitEElassId/seasonId/rarityE‰ã‚’ã€ã‚²ãƒ¼ãƒ å†ETraitTypeEE0ç¨®E‰ã¸ã©ãE½ã¨ã™ã‹ã€ã ã£ãŸã€E- å°åEè¦å‰‡ãŒæ›–æ˜§ãªã¾ã¾ã ã¨ã€ã‚¤ãƒ³ãƒE‚¯ã‚µã‚„UIã”ã¨ã«è§£é‡ˆãŒå‰²ã‚Œã¦ **replay / å…¬å¼æˆ¦ã‚ªãƒ³ãƒã‚§ãƒ¼ãƒ³æ±ºæ¸ˆãŒç ´ç¶»**ã™ã‚‹ã€E- ã•ã‚‰ã«ã€class/season/rarity ãŒã‚ªãƒ³ãƒã‚§ãƒ¼ãƒ³ã§å…¬é–‹ã•ã‚Œã¦ãE‚‹ä»¥ä¸Šã€ãã‚Œã‚’ã‚²ãƒ¼ãƒ æ€§Eˆç’°å¢E¨­è¨EãƒEƒƒã‚­äºˆç®—ãªã©E‰ã«æ¥ç¶šã§ãã‚‹ã€Œæ‹¡å¼µç‚¹ã€ã¨ã—ã¦ã€ãƒ«ãƒ¼ãƒ«ã‚»ãƒEƒˆã«å«ã‚ã¦ãŠããŸã‹ã£ãŸã€E
### What
- `RulesetConfigV1.synergy.traitDerivation`EEyanoTraitDerivationConfigV1E‰ã‚’è¿½åŠ ã€E- TSå‚çEå®Ÿè£E« Nyanoç”¨ãƒ˜ãƒ«ãƒ‘ã‚’è¿½åŠ EEpackages/triad-engine/src/nyano.ts`E‰ï¼E  - `DEFAULT_NYANO_TRAIT_DERIVATION_CONFIG_V1`
  - `deriveTraitTypeFromNyanoTraitV1(...)`
  - `makeCardDataFromNyano(...)`EEn-chain read â†ECardData ã®çµE¿ç«‹ã¦EE- ãƒEƒ•ã‚©ãƒ«ãƒˆãƒ«ãƒ¼ãƒ«ã‚»ãƒEƒˆã« `traitDerivation` ã‚’åŒæ¢±Eˆãƒ«ãƒ¼ãƒ«ã®â€œæ¨™æº–è§£é‡ˆâ€ã‚’å›ºå®šï¼‰ã€E- ä»•æ§˜è¿½åŠ Eš`Nyano_Triad_League_TRAIT_DERIVATION_SPEC_v1_ja.md`
- æ—¢å­˜ä»•æ§˜æ›´æ–°Ešruleset/transcript/trait-effects ãŒå°åEãƒ«ãƒ¼ãƒ«ã‚’å‚ç…§ã™ã‚‹ã‚ˆã†ã«è¿½è¨˜ã€E- ãƒE‚¹ãƒˆè¿½åŠ Ešrarityã”ã¨ã®å°åEåˆE²ã¨ `makeCardDataFromNyano` ã®çµE¿ç«‹ã¦ã‚’ã‚´ãƒ¼ãƒ«ãƒEƒ³åŒ–ã€E
### Verify
- `pnpm -C packages/triad-engine test`
- `pnpm -C packages/triad-engine build`
- `docs/02_protocol/*` / `docs/99_dev/*` ã®å·®åˆE¢ºèªE
## 2026-02-02 â€Ecommit-0007

### Why
- Design v2.0 ã®ã€Œãƒ•ã‚©ãƒ¼ãƒ¡ãƒ¼ã‚·ãƒ§ãƒ³ãƒœãEãƒŠã‚¹EE.3.3E‰ã€ãEã€ãƒ‡ãƒE‚­æ§‹ç¯‰ã‚’â€œå¼·ã‚«ãƒ¼ãƒ‰ãEå¯E›é›E‚â€ã‹ã‚‰è„±å´ã•ã›ã‚‹ä¸­æ ¸ãªã®ã§ã€æ—©ã‚ã«å‚çEå®Ÿè£E¸è½ã¨ã—ãŸã‹ã£ãŸã€E- ã¾ãESeason 3 ã®ä¾‹ï¼ˆäº”è¡Œèª¿å’ŒãEãƒ¼ãƒŠã‚¹3å€E/ Light+Shadow=æ—¥é£Ÿï¼‰ã«ã‚ã‚‹ã‚ˆã†ã«ã€ã‚·ãƒ¼ã‚ºãƒ³ç’°å¢E¼Eayer4E‰ãŒ **å€ç‡ãƒ»è¿½åŠ åŠ¹æE* ã¨ã—ã¦ä¸Šæ›¸ãã§ãã‚‹åœŸå°ãŒå¿E¦ã ã£ãŸã€E- â€œé‹å–¶ãŒã„ãªãã¦ã‚‚ç››ã‚Šä¸ŠãŒã‚‹â€ã«ã¯ã€ç¬¬ä¸‰è€EŒãƒªãƒ—ãƒ¬ã‚¤ã‚E’°å¢EEæã‚’ä½œã‚Œã‚‹ã‚ˆãE€ã©ã®formationãŒæœ‰åŠ¹ã ã£ãŸã‹ã‚’çµæœã«å«ã‚ã¦ãŠãã®ãŒé‡è¦ã€E
### What
- `RulesetConfigV1.synergy.formationBonuses` ã‚’è¿½åŠ EEata-drivenE‰ã€E- v1ã®æœ€å°ã‚»ãƒEƒˆã¨ã—ã¦ã€Eã¤ã®formationã‚’å®Ÿè£E¼E  - **äº”è¡Œèª¿å’Œï¼Eive Elements HarmonyEE*EE    - æ¡ä»¶EšFlame/Aqua/Earth/Wind/Thunder ãŒãƒ‡ãƒE‚­ã«æE†
    - åŠ¹æœï¼šcomboBonusEEomentum/DominationE‰ãE triadPlus ã‚E`comboBonusScale` å€E  - **æ—¥é£Ÿï¼EclipseEE*EE    - æ¡ä»¶EšLight ã¨ Shadow ãŒãƒ‡ãƒE‚­ã«æE†
    - åŠ¹æœï¼Eulesetã§ON/OFFå¯èƒ½E‰ï¼E      - LightãŒè­¦æˆ’ãEãƒ¼ã‚¯ã® -1 ã‚’ç„¡åŠ¹åŒE      - Shadowã‚ELightå…‰æºã¨ã—ã¦æ‰±ãE€Light aura ã‚’ç™ºç”Ÿã•ã›ã‚‹
- `MatchResult.formations` ã‚’è¿½åŠ ã—ã€UI/è§£æãŒ â€œé‹å–¶ãªã—â€Eã§ã‚‚ä½œã‚Šã‚E™ãE½¢ã«ã—ãŸã€E- ä»•æ§˜è¿½åŠ EE  - `Nyano_Triad_League_FORMATION_BONUS_SPEC_v1_ja.md`
- æ—¢å­˜ä»•æ§˜è¿½å¾“ï¼E  - ruleset spec / transcript spec ã‚Eformation ä»•æ§˜ã«è¿½å¾“ã•ã›ãŸã€E- ãƒE‚¹ãƒˆè¿½åŠ EE  - äº”è¡Œèª¿å’Œã«ã‚ˆã‚‹ comboBonus å€ç‡é©ç”¨ãŒæ¬¡ã‚¿ãƒ¼ãƒ³ã«åæ˜ ã•ã‚Œã‚‹ã“ã¨
  - æ—¥é£Ÿã«ã‚ˆã‚Š Light ãŒè­¦æˆ’ãEãƒ¼ã‚¯ã‚’è¸ã‚“ã§ã‚Etriad ãŒä¸‹ãŒã‚‰ãªãE“ã¨

### Verify
- `pnpm -C packages/triad-engine test`
- `pnpm -C packages/triad-engine build`
- ä»•æ§˜å·®åˆE¼š`docs/02_protocol/*` / `docs/99_dev/*` ã®æ›´æ–°ç¢ºèªE
## 2026-02-02 â€Ecommit-0008

### Why
- â€œé‹å–¶ãŒã„ãªãã¦ã‚‚å›ã‚‹â€ãŸã‚ã«ã¯ã€ã‚³ãƒŸãƒ¥ãƒ‹ãƒ†ã‚£ãŒææ¡ˆã™ã‚Eruleset ãE**è¡çªã›ãšã«è­˜åˆ¥**ã§ãã‚‹å¿E¦ãŒã‚ã‚‹ã€E- JSONã®ã‚ˆã†ãªæ›–æ˜§ãªã‚·ãƒªã‚¢ãƒ©ã‚¤ã‚ºã ã¨ã€è¨€èªå·®Eˆã‚­ãƒ¼é EEæ•°å€¤è¡¨è¨˜ãEUnicodeç­‰ï¼‰ã§ **åŒã˜ãƒ«ãƒ¼ãƒ«ãªã®ã«IDãŒåEè£E* ã—ã‚„ã™ã„ã€E- å°E¥ã‚ªãƒ³ãƒã‚§ãƒ¼ãƒ³ã« RulesetRegistry ã‚’ç½®ãå ´åˆã‚‚ã€Solidityå´ã§åŒã˜IDã‚’è¨ˆç®—ã§ãã‚‹å½¢EE fixed ABI encodingE‰ãŒæœ›ã¾ã—ã„ã€E
### What
- `computeRulesetIdV1(ruleset)` ã‚’è¿½åŠ EESå‚çEå®Ÿè£E¼‰ã€E  - `rulesetId = keccak256(abi.encode(RulesetConfigV1Canonical))` ã‚’å›ºå®šã€E  - ç„¡åŠ¹åŒ–ã‚»ã‚¯ã‚·ãƒ§ãƒ³EEnabled=falseE‰ãE **ã‚¼ãƒ­åŒ–ã—ã¦æ­£è¦åŒ–**EˆåŒã˜æŒ™å‹•ã§IDãŒåEè£‚ã—ãªãE¼‰ã€E  - äº”è¡Œèª¿å’ŒãE `requiredElements` ã¯é›Eˆã¨ã—ã¦æ‰±ãE€E*é Eºã‚’ç„¡è¦E*EEodeæ˜E E«ã‚½ãƒ¼ãƒˆï¼‰ã€E- ä»•æ§˜è¿½åŠ EE  - `Nyano_Triad_League_RULESET_ID_SPEC_v1_ja.md`
  - RULESET_CONFIG_SPEC / TRANSCRIPT_SPEC ã‚’å‚ç…§è¿½è¨E- ãƒE‚¹ãƒˆè¿½åŠ EE  - default rulesetId ã® test vector ã‚’å›ºå®E  - ç„¡åŠ¹åŒ–ã‚»ã‚¯ã‚·ãƒ§ãƒ³ã®æ­£è¦åŒ–ãŒåŠ¹ãE¦ãE‚‹ã“ã¨
  - requiredElements ã®é EºãŒIDã«å½±éŸ¿ã—ãªãE“ã¨
...
## 2026-02-08 â€Ecommit-0083: /stream parserçµ±ä¸€Eˆç¥¨å‰²ã‚Œã‚¼ãƒ­EE
### Why
- Stream.tsx ã« 9 å€‹ãEé‡è¤E–¢æ•°ãŒã‚ã‚Šã€triad_vote_utils / triad_viewer_command ã¨åŒã˜è¨ˆç®—ã‚’ç‹¬è‡ªå®Ÿè£E—ã¦ãEŸã€E- `parseChatMove()` ãŒç‹¬è‡ªãƒ‘ãEã‚¹å®Ÿè£E§ã€`parseViewerMoveTextLoose()` ã¨ç•°ãªã‚‹æ­£è¦åŒ–ã‚’ã™ã‚‹ãŸã‚ç¥¨å‰²ã‚ŒãŒç™ºç”Ÿã—ã¦ãEŸã€E
### What
- `triad_viewer_command.ts` ã« `parseChatMoveLoose()` ã‚’è¿½åŠ ã€‚canonical / legacy / shorthand å…¨ã¦ã‚E`formatViewerMoveText()` ã§åŒä¸€ã‚­ãƒ¼ã«æ­£è¦åŒ–ã€E- Stream.tsx ã‹ã‚‰ 9 å€‹ãEé‡è¤E–¢æ•°ã‚’å‰Šé™¤ã€triad_vote_utils / triad_viewer_command ã® import ã«ç½®æ›ã€E- `parseChatMove()` ã‚E`parseChatMoveLoose()` ã«ç½®æ›ã€‚`ParsedMove` åEâ†E`ViewerMove` ã«çµ±ä¸€ã€E- `buildStateJsonContent()` / `buildAiPrompt()` ã‚E`computeStrictAllowed()` / `computeToPlay()` ã«åˆE›¿ã€E- Match.tsx ã®ã‚¹ãƒãEãƒˆã‚¯ã‚©ãƒ¼ãƒˆï¼E+201C/U+201DE‰ãƒ“ãƒ«ãƒ‰ã‚¨ãƒ©ãƒ¼ã‚’ä¿®æ­£ã€E
### Verify
- `pnpm build:web` æˆåŠŸ


## 2026-02-08 â€Ecommit-0084: ã‚¨ãƒ©ãƒ¼è¡¨ç¤ºå¸¸è¨­ + flipçE”±è¡¨ç¤ºçµ±ä¸€

### Why
- å¤–éƒ¨é€£æºEEarudoç­‰ï¼‰ãEæˆåŠŸ/å¤±æ•—ãŒä¸€æ™‚çš„ãª toast ã§ã—ã‹è¡¨ç¤ºã•ã‚Œãšã€ã‚¹ãƒˆãƒªãƒ¼ãƒãEãŒè¦‹é€E—ã‚E™ã‹ã£ãŸã€E- Overlay ã® flip çE”±è¡¨ç¤ºãŒæ‰‹å‹•ãE flipStats é›E¨ˆã§ã€TurnLog ã® FlipTraceBadges ã¨ä¸€è‡´ã—ãªã‹ã£ãŸã€E
### What
- StreamOperationsHUD ã« `ExternalResult` å‹ã¨ `ExternalStatusRow` ã‚³ãƒ³ãƒãEãƒãƒ³ãƒˆã‚’è¿½åŠ ã€E- Stream.tsx ã« `lastExternalResult` state ã‚’è¿½åŠ ã€`sendNyanoWarudo()` ã§è¨˜éŒ²ã€E- `OverlayStateV1` ã« `externalStatus` ãƒ•ã‚£ãƒ¼ãƒ«ãƒ‰ã‚’è¿½åŠ Eˆäº’æ›æ‹¡å¼µE‰ã€E- Overlay.tsx ã®æ‰‹å‹• flipStats ãƒãƒƒã‚¸ â†E`FlipTraceBadges` ã‚³ãƒ³ãƒãEãƒãƒ³ãƒˆã«ç½®æ›ã€E- Overlay.tsx ã®æ‰‹å‹• "Why:" ã‚»ã‚¯ã‚·ãƒ§ãƒ³ â†E`flipTracesSummary()` ã«çµ±ä¸€ã€E
### Verify
- `pnpm build:web` æˆåŠŸ


## 2026-02-08 â€Ecommit-0085: Overlay HUD è¦–èªæ€§ + UI ã‚¯ã‚ªãƒªãƒE‚£ã‚¢ãƒEE

### Why
- OBS controls=0 ãƒ¢ãƒ¼ãƒ‰ã§ 720p/1080p è¡¨ç¤ºæ™‚ã«æ–E­—ãŒå°ã•ã™ãã¦åˆ¤èª­å›°é›£ã ã£ãŸã€E- ãƒ‘ãƒãƒ«èƒŒæ™¯ã®é€éãŒå¼·ãã€EEä¿¡æ˜ åƒã¨é‡ãªã‚‹ã¨æ–E­—ãŒè¦‹ãˆã«ãã‹ã£ãŸã€E
### What
- ScoreBar ã« `size` prop ã‚’è¿½åŠ EEsm" | "md" | "lg"E‰ã€E- Overlay OBS ãƒ¢ãƒ¼ãƒ‰ãEãƒ•ã‚©ãƒ³ãƒˆéšå±¤ã‚’ä¸€å¾‹å¼•ãä¸Šã’EE0pxâ†E2px, 11pxâ†E2px, xsâ†’sm, smâ†’baseE‰ã€E- ãƒ‘ãƒãƒ«èƒŒæ™¯ `bg-white/70` â†E`bg-white/90`EEBS ãƒ¢ãƒ¼ãƒ‰ï¼‰ã€E- toPlay è¡¨ç¤ºã‚E`to-play-pill` ã‚³ãƒ³ãƒãEãƒãƒ³ãƒˆåŒ–EˆãEãƒ¬ã‚¤ãƒ¤ãƒ¼ã‚«ãƒ©ãƒ¼ä»˜ãE‰ã€E- ã‚»ãƒ«åº§æ¨™ãƒ©ãƒ™ãƒ«ã‚’å¸¸æ™‚è¡¨ç¤ºã«å¤‰æ›´ã€‚ãEãƒ¼ãƒEgap ã‚EOBS ãƒ¢ãƒ¼ãƒ‰ã§æ‹¡å¤§ã€E- index.css ã« `vote-countdown-inline`, `to-play-pill` CSS ã‚³ãƒ³ãƒãEãƒãƒ³ãƒˆã‚’è¿½åŠ ã€E
### Verify
- `pnpm build:web` æˆåŠŸ


## 2026-02-12 â€Ecommit-0086: Quick Play å°ç·šãƒ†ãƒ¬ãƒ¡ãƒˆãƒªè¿½åŠ EEomeâ†’åEæ‰‹éEç½®EE
### Why
- UX ã‚¹ã‚³ã‚¢ã‚«ãƒ¼ãƒEB-1ã€ŒHome ã‹ã‚‰è©¦åˆé–‹å§‹ã¾ã§10ç§’ä»¥å†E€ãŒæœªè¨ˆæ¸¬ã§ã€æ”¹å–EEãƒ«ãƒ¼ãƒ—ã‚’å›ã—ã«ãã‹ã£ãŸã€E- æ—¢å­˜ãE `first_place_ms` ã¯ Match ãƒšãEã‚¸èµ·ç‚¹ã®ãŸã‚ã€Home CTA ã‹ã‚‰ã®ä½“é¨“æ™‚é–“ã‚’ç›´æ¥è©•ä¾¡ã§ããªã‹ã£ãŸã€E
### What
- `telemetry.ts` ã« `quickplay_to_first_place_ms` ã‚’è¿½åŠ EEession + Cumulative å¹³åE¼‰ã€E- Home ã®ã€ŒğŸ® ã™ãéŠãEã€æŠ¼ä¸‹æ™‚ã« `markQuickPlayStart()` ã‚’è¨˜éŒ²ã—ã€Match å´ã®åˆå›é…ç½®ã§æ¶ˆè²»ã—ã¦è¨ˆæ¸¬ã™ã‚‹ã‚ˆã†ã«ã—ãŸã€E- Home > Settings ã® UX Telemetry ãƒ‘ãƒãƒ«ã« `Avg quick-play to first place` ã‚’è¡¨ç¤ºè¿½åŠ ã€E- ãƒE‚¹ãƒˆè¿½åŠ EE  - Home ãƒãEã‚«ãƒ¼ã‚ã‚Šã§è¨ˆæ¸¬ã•ã‚Œã‚‹ã“ã¨
  - ãƒãEã‚«ãƒ¼ãEå›ã§æ¶ˆè²»ã•ã‚Œã‚‹ã“ã¨
- ãƒ‰ã‚­ãƒ¥ãƒ¡ãƒ³ãƒˆæ›´æ–°EE  - `UX_SCORECARD` ã® B-1 ã‚’ã€Œè¨ˆæ¸¬å¯èƒ½ã€ã«æ›´æ–°
  - ãƒEƒ¬ãƒ¡ãƒˆãƒªä¸€è¦§ã¸ `quickplay_to_first_place_ms` ã‚’è¿½åŠ 
...
## 2026-02-12 â€Ecommit-0087: Home LCP ãƒ­ãƒ¼ã‚«ãƒ«è¨ˆæ¸¬è¿½åŠ EE-3EE
### Why
- UX ã‚¹ã‚³ã‚¢ã‚«ãƒ¼ãƒEG-3EECP < 2.5sE‰ãŒæœªè¨ˆæ¸¬ã§ã€æ”¹å–E‰å¾ŒãEæ¯”è¼EŒã§ããªã‹ã£ãŸã€E- æ—¢å­˜ãE Home Settings ãƒEƒ¬ãƒ¡ãƒˆãƒªã«ã€ãƒ‘ãƒ•ã‚©ãƒ¼ãƒãƒ³ã‚¹ã®ä¸­æ ¸æŒE¨™ã‚’åŒã˜å°ç·šã§è¡¨ç¤ºã—ãŸã‹ã£ãŸã€E
### What
- `telemetry.ts` ã® cumulative stats ã« `avg_home_lcp_ms` ã‚’è¿½åŠ ã€E- `recordHomeLcpMs()` ã‚’è¿½åŠ ã—ã€Home ãƒšãEã‚¸ã® LCP ã‚’ãƒ­ãƒ¼ã‚«ãƒ«é›E¨ˆã§ãã‚‹ã‚ˆã†ã«ã—ãŸã€E- Home ã§ `PerformanceObserver`EElargest-contentful-paint`E‰ã‚’ç›£è¦–ã—ã€`visibilitychange/pagehide` ãE6 ç§’ãƒ•ã‚©ãƒ¼ãƒ«ãƒãƒƒã‚¯ã§è¨˜éŒ²ã€E- Home > Settings ã®ãƒ¡ãƒˆãƒªã‚¯ã‚¹ã« `Avg Home LCP` ã‚’è¿½åŠ ã€E- ãƒE‚¹ãƒˆè¿½åŠ EE  - Home LCP å¹³åEEè¨ˆç®E  - ä¸æ­£å€¤EEaN / è² æ•° / InfinityE‰ã‚’ç„¡è¦–ã™ã‚‹æŒ™å‹E- `UX_SCORECARD` ã‚’æ›´æ–°ã—ã€G-3 ã‚’ã€Œè¨ˆæ¸¬å¯èƒ½ã€ã«å¤‰æ›´ã€E
### Verify
- `pnpm -C apps/web test`
- `pnpm -C apps/web lint`
- `pnpm -C apps/web build`
...
## 2026-02-12 â€Ecommit-0088: UXç›®æ¨™ã‚¹ãƒŠãƒƒãƒ—ã‚·ãƒ§ãƒEƒˆè¡¨ç¤º + quick-playè¨ˆæ¸¬ã®å E‰¢åŒE
### Why
- ãƒEƒ¬ãƒ¡ãƒˆãƒªå€¤ãŒå¢—ãˆã¦ããŸãŸã‚ã€EEä¿¡å‰ãƒã‚§ãƒE‚¯ã§ã€Œç›®æ¨™ã‚’æº€ãŸã—ã¦ãE‚‹ã‹ã€ã‚’å³åˆ¤å®šã§ãã‚‹è¡¨ç¤ºãŒå¿E¦ã ã£ãŸã€E- `quickplay_to_first_place_ms` ã¯å¤ãE–‹å§‹æ™‚åˆ»ãŒæ®‹ã‚‹ã¨å¤–ã‚Œå€¤ã«ãªã‚Šå¾—ã‚‹ãŸã‚ã€ç•°å¸¸å€¤ã‚¬ãƒ¼ãƒ‰ã‚’å…¥ã‚Œã¦èª¤åˆ¤å®šã‚’é˜²ããŸã‹ã£ãŸã€E
### What
- `telemetry.ts` ã« `evaluateUxTargets(stats)` ã‚’è¿½åŠ ã—ã€A-1/B-1/B-4/G-3 ã® PASS/FAIL/INSUFFICIENT ã‚’ç®—åEå¯èƒ½ã«ã—ãŸã€E- Home > Settings ã« `UX Target Snapshot` ã‚’è¿½åŠ ã—ã€ä¸Šè¨Eé E›®ã‚’ç›®æ¨™å€¤ã¨ç¾åœ¨å€¤ã¤ãã§å¯è¦–åŒ–ã€E- quick-play è¨ˆæ¸¬ã«ä¸Šé™EE0åˆE¼‰ã‚’è¿½åŠ ã—ã€å¤ãE–‹å§‹æ™‚åˆ»ã«ã‚ˆã‚‹å¤–ã‚Œå€¤ã‚’ç„¡è¦–ã™ã‚‹ã‚ˆãE«ã—ãŸã€E- ãƒE‚¹ãƒˆè¿½åŠ EE  - stale quick-play marker ã‚’ç„¡è¦–ã™ã‚‹ã“ã¨
...
## 2026-02-12 â€Ecommit-0089: UXè¨ˆæ¸¬ãƒ­ã‚°ã®ã‚³ãƒ”ãEå°ç·E+ Playtest Log ãƒEƒ³ãƒ—ãƒ¬

### Why
- è¨ˆæ¸¬å€¤ãŒè¦‹ãˆã‚‹ã‚ˆãE«ãªã£ãŸä¸€æ–¹ã§ã€`UX_SCORECARD` é‹ç”¨ã®è¨˜éŒ²è»¢è¨˜ãŒæ‰‹ä½œæ¥­ã§ã€ç¶™ç¶šã—ã«ãã‹ã£ãŸã€E- é…ä¿¡å‰Eæ”¹ä¿®å¾Œã«åŒã˜ãƒ•ã‚©ãƒ¼ãƒãƒƒãƒˆã§æ¯”è¼E§ãã‚‹ãƒ­ã‚°å‡ºåŠ›ã‚’ã€UIã‹ã‚‰1ã‚¯ãƒªãƒE‚¯ã§å–å¾—ã—ãŸã‹ã£ãŸã€E
### What
- `telemetry.ts` ã«ä»¥ä¸‹ã‚’è¿½åŠ EE  - `buildUxTelemetrySnapshot(stats)`EEimestamp + stats + target checksEE  - `formatUxTelemetrySnapshotMarkdown(snapshot)`EEPLAYTEST_LOG.md` è²¼ã‚Šä»˜ã‘å½¢å¼ï¼E- Home Settings ã® `UX Telemetry` ã« `Copy Snapshot` ãƒœã‚¿ãƒ³ã‚’è¿½åŠ ã€E  - ã‚¯ãƒªãƒE‚¯ã§ markdown ã‚’ã‚¯ãƒªãƒEEãƒœãEãƒ‰ã«ã‚³ãƒ”ãEã—ã€`docs/ux/PLAYTEST_LOG.md` ã¸ã®è²¼ã‚Šä»˜ã‘ã‚’æ¡ˆåEã€E- `docs/ux/PLAYTEST_LOG.md` ã‚’æ–°è¦ä½œæEã—ã€è¨˜éŒ²ãƒEƒ³ãƒ—ãƒ¬ã‚’è¿½åŠ ã€E- `UX_SCORECARD` ã®è¨˜éŒ²å…ˆè¡¨è¨˜ã‚’ãƒEƒ³ãƒ—ãƒ¬ä½œæEæ¸ˆã¿çŠ¶æ…‹ã¸æ›´æ–°ã€E- ãƒE‚¹ãƒˆè¿½åŠ EE  - snapshot ç”ŸæEã® timestamp/shape
  - markdown æ•´å½¢å†E®¹
...
## 2026-02-12 â€Ecommit-0090: lint warning 0 åŒ–ï¼EebEE
### Why
- `pnpm -C apps/web lint` ã«æ—¢çŸ¥ warning ãEä»¶æ®‹ã£ã¦ãŠã‚Šã€æ—¥å¸¸ã®æ¤œè¨¼ã§ãƒã‚¤ã‚ºã«ãªã£ã¦ãEŸã€E- warning ã‚’æ”¾ç½®ã™ã‚‹ã¨ã€æ–°è¦Ewarning ã®æ¤œçŸ¥æ€§ãŒè½ã¡ã‚‹ãŸã‚æ—©ã‚ã«è§£æ¶ˆã—ãŸã‹ã£ãŸã€E
### What
- `apps/web/src/engine/renderers/pixi/cellAnimations.ts`
  - æœªä½¿ç”¨å¼•æ•° `cellH` ã‚E`_cellH` ã«å¤‰æ›´EEPIäº’æ›ã‚’ç¶­æŒã—ã¦ lint æº–æ‹ E‰ã€E- `apps/web/src/engine/__tests__/cellAnimations.test.ts`
  - æœªä½¿ç”¨ã®åEimport `CellAnimFrame` ã‚’å‰Šé™¤ã€E
### Verify
- `pnpm -C apps/web test`
- `pnpm -C apps/web lint`
- `pnpm -C apps/web typecheck`
...
## 2026-02-12 â€Ecommit-0091: UX snapshot ã«ç’°å¢E‚³ãƒ³ãƒE‚­ã‚¹ãƒˆã‚’è¿½åŠ 

### Why
- åŒã˜æŒE¨™ã§ã‚‚ç«¯æœ«ã‚E¡¨ç¤ºã‚µã‚¤ã‚ºã§ä½“é¨“å€¤ãŒå¤‰ã‚ã‚‹ãŸã‚ã€snapshotæ¯”è¼E™‚ã«å®Ÿè¡Œç’°å¢E‚’æ®‹ã™å¿E¦ãŒã‚ã£ãŸã€E- `PLAYTEST_LOG.md` ã«è²¼ã‚‹æƒ…å ±ã‚’å¢—ã‚„ã—ã€å¾Œã‹ã‚‰ã€Œãªãœå·®ãŒåEãŸã‹ã€ã‚’è¿½è·¡ã—ã‚„ã™ãã—ãŸã‹ã£ãŸã€E
### What
- `telemetry.ts` ã« `UxTelemetryContext` ã‚’è¿½åŠ ã—ã€snapshotã¸ `context` ã‚’å«ã‚ã‚‰ã‚Œã‚‹ã‚ˆã†ã«ã—ãŸã€E- `formatUxTelemetrySnapshotMarkdown()` ã‚’æ‹¡å¼µã—ã€`route / viewport / language / userAgent` ã‚’åEåŠ›ã™ã‚‹ã‚ˆãE«ã—ãŸã€E- Home ã® `Copy Snapshot` ã§ãƒ–ãƒ©ã‚¦ã‚¶æƒE ±ã‚’åé›E—ã¦ snapshot ã«åŸ‹ã‚è¾¼ã‚€ã‚ˆã†ã«ã—ãŸã€E- `PLAYTEST_LOG.md` ã®ãƒEƒ³ãƒ—ãƒ¬ã« context ä¾‹ã‚’è¿½è¨˜ã€E- ãƒE‚¹ãƒˆè¿½åŠ EE  - context ã‚ã‚Š snapshot ç”ŸæE
  - markdown ã® context å‡ºåŠE- e2e `home.spec.ts` ã‚’æ›´æ–°ã—ã€Settings å†EE `Copy Snapshot` / `UX Target Snapshot` è¡¨ç¤ºã‚’æ¤œè¨¼å¯¾è±¡ã«è¿½åŠ ã€E
### Verify
- `pnpm -C apps/web test`
- `pnpm -C apps/web lint`
- `pnpm -C apps/web typecheck`
...
### Why
- Commit0112 ã§ãƒ­ãƒ¼ã‚«ãƒ« import UI ã¯å…¥ã£ãŸãŒã€åEåŠ›ãŒæ‰‹è²¼ã‚Šå‰æã§é‹ç”¨è² è·ãŒæ®‹ã£ã¦ãEŸã€E- pointsDelta ã‚E`settled_attested` ã¨ã—ã¦æ‰±ãE°ç·šã«ã¯ã€ç½²åæ¤œè¨¼æ¸ˆã¿ record ã‚’é¸ã¹ã‚‹ãƒ¢ãƒ¼ãƒ‰ãŒå¿E¦ã ã£ãŸã€E- æ—¢å­˜ãE season points ç§»è¡Œã‚’å£Šã•ãšã«ã€`fast import` ã¨ `verified import` ã‚’æ®µéšå°åEã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/lib/settled_points_import.ts`
  - `parseVerifiedLadderRecordsImportJson(...)` ã‚’è¿½åŠ ã€E  - payload å½¢å¼E`{ domain, records }` ã‚’å—ã‘å–ã‚Šã€`verifyLadderMatchRecordV1(...)` ã§ record ã”ã¨ã«æ¤œè¨¼ã€E  - issue code `attestation_invalid` ã‚’è¿½åŠ ã—ã€æ¤œè¨¼å¤±æ•—ç†ç”±ã‚’é›†ç´E€E  - duplicate åˆ¤å®šãƒ­ã‚¸ãƒE‚¯ã‚E`pushUniqueSettledEvent(...)` ã«å…±é€šåŒ–ã€E- `apps/web/src/lib/__tests__/settled_points_import.test.ts`
  - verified import ã® schema ä¸æ­£ã‚±ãƒ¼ã‚¹ãƒ»attestation å¤±æ•—ã‚±ãƒ¼ã‚¹ã‚’è¿½åŠ ã€E- `apps/web/src/pages/Events.tsx`
  - import mode åˆE›¿ UI ã‚’è¿½åŠ EE    - `Settled events (fast)`
    - `Verified records (domain + signatures)`
  - `/game/settled_events.json` è‡ªå‹•èª­è¾¼ãƒœã‚¿ãƒ³ã‚’è¿½åŠ ã€E  - mode ã«å¿œã˜ã¦ parser ã‚’åEã‚Šæ›¿ãˆã€åŒãEapply ãƒ•ãƒ­ãƒ¼ã§ local attempts ã«åæ˜ ã€E- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md`
  - Commit0113 å®ŒäºE‚’è¿½è¨˜ã—ã€Doing ã‚’ã€ŒãƒãƒE‚¯ã‚¨ãƒ³ãƒ‰çµŒç”±ã®è‡ªå‹•ä¾›çµ¦ã¨å®šæœŸåŒæœŸã€ã¸æ›´æ–°ã€E
### Verify
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web lint`
...
### Why
- DEV_TODO ã® Doing ã§ã¯ã€Phase 4 ã® pointsDelta é€£æºã‚EURL æ‰‹åEåŠ›ã‹ã‚E`on-chain settled event` å–ã‚Šè¾¼ã¿ã¸é€²ã‚ã‚‹æ®µéšã ã£ãŸã€E- æ—¢å­˜ãƒ•ãƒ­ãƒ¼ã¯ Replay URL ã® `pda` ä¾å­˜ãEãŸã‚ã€å¾Œã‹ã‚Esettled event ã‚’ä¸€æ‹¬åæ˜ ã™ã‚‹å°ç·šãŒãªãã€season points ã®ç§»è¡ŒåŠ¹çEŒä½ã‹ã£ãŸã€E- ãƒ­ãƒ¼ã‚«ãƒ«ä¿å­˜ãE event attempts ã«å¯¾ã—ã¦ã€å®‰åEã« `pointsDeltaA` ã‚’é©ç”¨ã™ã‚‹ã«ã¯ã€`matchId` ä¸€è‡´ã ã‘ã§ãªãEwinner / tiles æ•´åˆãƒã‚§ãƒE‚¯ãŒå¿E¦ã ã£ãŸã€E
### What
- `apps/web/src/lib/settled_points_import.ts` ã‚’è¿½åŠ ã€E  - å…¥åŠEschema å¯¾å¿E
    - settled event é…åE
    - `{ settledEvents: [...] }`
    - `{ records: [{ settled: ... }] }`
  - `validateLadderMatchSettledEventV1(...)` ã‚’ä½¿ã£ã¦ settled event ã‚’æ¤œè¨¼ã€E  - `matchId` å˜ä½ã§æ­£è¦åŒ–ã—ã€ç«¶åEduplicate ã‚Eissue ã¨ã—ã¦å ±å‘Šã€E  - `applySettledPointsToAttempts(...)` ã§ãƒ­ãƒ¼ã‚«ãƒ« attempt ã¸é©ç”¨:
    - no local / winner mismatch / tiles mismatch / draw ã‚’å®‰åEã«ã‚¹ã‚­ãƒEE
    - æ•´åˆã—ãEattempt ã®ã¿ `pointsDeltaA` + `pointsDeltaSource=settled_attested` ã‚’æ›´æ–°
- `apps/web/src/pages/Events.tsx`
  - `Settled points import (local)` UI ã‚’è¿½åŠ EESONè²¼ã‚Šä»˜ã‘ã€E©ç”¨ã€åEåŠ›ã‚¯ãƒªã‚¢E‰ã€E  - import çµæœã‚µãƒãƒªEEnput/valid/updated/matched/unchanged/no-local/mismatchE‰ã¨ issue æŠœç²‹è¡¨ç¤ºã‚’è¿½åŠ ã€E  - `Apply settled JSON` å®Ÿè¡Œæ™‚ã«æ›´æ–°å¯¾è±¡ attempt ã‚E`upsertEventAttempt(...)` ã§æ°¸ç¶šåŒ–ã€E  - My Pawprints ä¸€è¦§ã« `deltaA` ãƒãƒƒã‚¸è¡¨ç¤ºã‚’è¿½åŠ ã€E- `apps/web/src/lib/__tests__/settled_points_import.test.ts`
  - parseEˆè¤E•°schemaE‰ãEduplicate conflictãƒ»applyEˆæ­£å¸¸æ›´æ–°/ä¸æ•´åEãƒ­ãƒ¼ã‚«ãƒ«æœªä¸€è‡´E‰ã‚’æ¤œè¨¼ã€E- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md`
  - Commit0112 å®ŒäºE‚’è¿½è¨˜ã—ã€Doing ã‚’ã€Œå–å¾—èEå‹•åŒ–ã¨ç½²åæ¤œè¨¼ãƒ•ãƒ­ãƒ¼çµ±åˆã€ã¸æ›´æ–°ã€E
### Verify
- `pnpm -C apps/web test -- src/lib/__tests__/settled_points_import.test.ts`
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web lint`
...
### Why
- DEV_TODO ã® Doingã€ŒpointsDelta é€£æºã¸æ®µéšæ‹¡å¼µã€ã«å¯¾ã—ã€ç¾çŠ¶ã® season points ã¯ provisional ãƒ«ãƒ¼ãƒ«ã®ã¿ã ã£ãŸã€E- on-chain settled event ã®è‡ªå‹•å–ã‚Šè¾¼ã¿å‰ã«ã€`pointsDeltaA` ã‚’å®‰åEã«å—ã‘å–ã£ã¦é›E¨ˆã«åæ˜ ã§ãã‚‹ç§»è¡Œãƒ¬ã‚¤ãƒ¤ãƒ¼ãŒå¿E¦ã ã£ãŸã€E- æ—¢å­˜å±¥æ­´ã¨ã®äº’æ›æ€§ã‚’å®ˆã‚‹ãŸã‚ã€Eƒ¨åˆEƒ‡ãƒ¼ã‚¿ã§é E½ãŒä¸å®‰å®šåŒ–ã—ãªãE¡ç”¨æ¡ä»¶ã‚’å›ºå®šã—ãŸã‹ã£ãŸã€E
### What
- `apps/web/src/lib/event_attempts.ts`
  ...
  - replay share URL ã« `pda`EEointsDeltaAE‰ã‚’è¿½åŠ ã§ãã‚‹ã‚ˆã†æ‹¡å¼µã€E- `apps/web/src/pages/Replay.tsx`
  - `?pda=` ã‚Eint32 ã§è§£æã€E  - Event attempt ä¿å­˜æ™‚ã« `pointsDeltaA` ã‚’ä¿æŒã€E  - share/canonical link ã§ã‚E`pda` ã‚’ç¶­æŒã€E- `apps/web/src/lib/season_archive.ts`
  - eventå˜ä½ãE `pointsDeltaTotal` / `pointsDeltaAttemptCount` / `pointsDeltaCoveragePercent` ã‚’è¿½åŠ ã€E  - archive markdown ã« delta åˆ—ã‚’è¿½åŠ ã€E- `apps/web/src/lib/season_progress.ts`
  - source æ¦‚å¿µEEprovisional` / `points_delta`E‰ã‚’è¿½åŠ ã€E  - eventå†E§ `pointsDeltaA` ãE00%æE£ãŸå ´åˆãEã¿ `points_delta` æ¡ç”¨ã€æœªå…E¶³ã¯ provisional ç¶­æŒã€E  - source mix é›E¨ˆã¨ markdown å‡ºåŠ›ã‚’è¿½åŠ ã€E- `apps/web/src/pages/Events.tsx`
  - progress ãƒ‘ãƒãƒ«ã« source mix è¡¨ç¤ºã‚’è¿½åŠ ã€E  - board ã« source badgeEEelta/provisionalE‰ã¨ coverage è¡¨ç¤ºã‚’è¿½åŠ ã€E  - eventè¡Œã« delta total / coverage ã‚’è¿½åŠ ã€E- Tests
  ...
  - pointsDelta å…¥åŠ›ãEé›E¨ˆãEæ¡ç”¨æ¡ä»¶ã‚’è¿½åŠ æ¤œè¨¼ã€E
### Verify
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web lint`
- `pnpm -C apps/web build`
- `pnpm -C apps/web test -- src/lib/__tests__/appUrl.test.ts src/lib/__tests__/season_archive.test.ts src/lib/__tests__/season_progress.test.ts`
  - ã“ãEå®Ÿè¡Œç’°å¢E§ã¯ `vite/vitest` èµ·å‹•æ™‚ã« `spawn EPERM` ã§å®Œèµ°ä¸å¯

## 2026-02-13 - commit-0110: local season points and reward-tier guidance on /events

### Why
- Phase 4 ã®æœªå®ŒäºE E›®ã€Œã‚·ãƒ¼ã‚ºãƒ³åˆ¶Eˆãƒ©ãƒ³ã‚­ãƒ³ã‚°/å ±é…¬/ã‚¢ãƒ¼ã‚«ã‚¤ãƒ–ï¼‰ã€ã«å¯¾ã—ã¦ã€archive ã¯å®Ÿè£E¸ˆã¿ã ãEranking/reward ã®å°ç·šãŒä¸è¶³ã—ã¦ãEŸã€E- å…¬å¼ãE on-chain `pointsDelta` é€£æºã‚’åEã‚Œã‚‹å‰æ®µã¨ã—ã¦ã€ãƒ­ãƒ¼ã‚«ãƒ«å±¥æ­´ã‹ã‚‰æ±ºå®šçš„ã«å†è¨ˆç®—ã§ãã‚‹æš«å®šé€²è¡ŒæŒ‡æ¨™ãŒå¿E¦ã ã£ãŸã€E- é›E¨ˆãƒ­ã‚¸ãƒE‚¯ã‚EUI ã«åŸ‹ã‚è¾¼ã‚€ã¨å°E¥ã® pointsDelta ç§»è¡Œæ™‚ã«å›å¸°ã—ã‚„ã™ã„ãŸã‚ã€pure function ã¨ã—ã¦åˆE›¢ã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/lib/season_progress.ts` ã‚’è¿½åŠ ã€E  - `Win +3 / Loss +1 / Event clear +2` ã®ãƒ­ãƒ¼ã‚«ãƒ« points ãƒ«ãƒ¼ãƒ«ã‚’å›ºå®šã€E  - reward tierEEookie/Bronze/Silver/Gold/LegendE‰åˆ¤å®šã‚’è¿½åŠ ã€E  - eventåˆ¥ points board ã‚’æ±ºå®šçš„ tie-break ã§ç”ŸæEã€E  - progress markdown å‡ºåŠ›ã‚’è¿½åŠ ã€E- `apps/web/src/pages/Events.tsx`
  - `Local season points (provisional)` ãƒ‘ãƒãƒ«ã‚’è¿½åŠ EEier / next / progress bar / hintE‰ã€E  - `Season points board`EEventåˆ¥E‰ã‚’è¿½åŠ ã€E  - `Copy summary` ã‚Earchive + progress ã®çµåˆå‡ºåŠ›ã¸æ‹¡å¼µã€E- `apps/web/src/lib/__tests__/season_progress.test.ts`
  - pointsç®—åEã€tieré·ç§»ã€tie-breakã€markdown å‡ºåŠ›ã‚’æ¤œè¨¼ã€E- Docs
...
### Why
- Phase 4 ã®å‚åŠ å°ç·šã§ã€Œæ–°è¦å‚åŠ è€E‘ã‘ãƒãƒ¥ãƒ¼ãƒˆãƒªã‚¢ãƒ«EEåˆE†è§£â†EåˆE‚åŠ E‰ã€ãŒæœªå®Ÿè£E ã£ãŸã€E- ãƒ«ãƒ¼ãƒ«ç¢ºèªã‹ã‚‰åEå›å¯¾æˆ¦ã¾ã§ã‚’çŸ­ãã—ã€E›¢è„±ã—ã‚„ã™ã„æœ€åˆãE1åˆE‚’ãƒ—ãƒ­ãƒ€ã‚¯ãƒˆåEã§è£œåŠ©ã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/lib/onboarding.ts` ã‚’æ–°è¦è¿½åŠ ã€E  - é€²æEã‚¹ãƒEƒƒãƒ—ï¼Eread_quick_guide` / `start_first_match` / `commit_first_move`E‰ã‚’å®šç¾©ã€E  - localStorage ã¸ã®èª­ã¿æ›¸ãã€å®ŒäºE•°é›E¨ˆã€åEå®ŒäºEˆ¤å®šã€reset ã‚’å®Ÿè£E€E- `apps/web/src/lib/__tests__/onboarding.test.ts` ã‚’æ–°è¦è¿½åŠ ã€E  - æ—¢å®šå€¤ã€E€²æ—æ°¸ç¶šåŒ–ã€å®ŒäºE•°åˆ¤å®šã€ç•°å¸¸payload fallbackã€reset ã‚’æ¤œè¨¼ã€E- `apps/web/src/pages/Home.tsx`
  - ã€ŒãEã˜ã‚ã¦ã®1åˆE‚¹ã‚¿ãƒ¼ãƒˆã€ãƒã‚§ãƒE‚¯ãƒªã‚¹ãƒEIã‚’è¿½åŠ ã€E  - 1åˆEƒ«ãƒ¼ãƒ«ãƒ¢ãƒ¼ãƒ€ãƒ«ã‚’è¿½åŠ ã—ã€è¡¨ç¤ºæ™‚ã« `read_quick_guide` ã‚’æ›´æ–°ã€E  - ã‚¯ã‚¤ãƒE‚¯å¯¾æˆ¦å°ç·šã§ `start_first_match` ã‚’æ›´æ–°ã—ã€E€²æ—ãƒªã‚»ãƒEƒˆæ“ä½œã‚’è¿½åŠ ã€E- `apps/web/src/pages/Match.tsx`
  - guest match é–‹å§‹æ™‚ã« `start_first_match` ã‚’æ›´æ–°ã€E  - æœ€åˆãEæ‰‹ãŒç¢ºå®šã—ãŸã‚¿ã‚¤ãƒŸãƒ³ã‚°EEturns.length >= 1`E‰ã§ `commit_first_move` ã‚’æ›´æ–°ã€E- `docs/00_handoff/Nyano_Triad_League_LONG_TERM_ROADMAP_v1_ja.md`
  - Phase 4 ã®ã€Œæ–°è¦å‚åŠ è€E‘ã‘ãƒãƒ¥ãƒ¼ãƒˆãƒªã‚¢ãƒ«ã€é E›®ã‚’å®ŒäºE«æ›´æ–°ã€E- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md`
...
- `pnpm -C apps/web test -- src/lib/__tests__/onboarding.test.ts`
  - ã“ãEå®Ÿè¡Œç’°å¢E§ã¯ `vite/vitest` èµ·å‹•æ™‚ã« `spawn EPERM` ãŒç™ºç”Ÿã—å®Œèµ°ä¸å¯

## 2026-02-12 - commit-0108: stream moderation controls (NG words / ban / slow mode)

### Why
- Phase 4 ã®æœªå®ŒäºE E›®ã€Œãƒ¢ãƒEƒ¬ãƒ¼ã‚·ãƒ§ãƒ³æ©ŸèEEEGãƒ¯ãƒ¼ãƒ‰ã€BANã€ã‚¹ãƒ­ãƒ¼ãƒ¢ãƒ¼ãƒ‰é€£æºE‰ã€ãŒ `/stream` ã«ä¸è¶³ã—ã¦ãEŸã€E- æ—¢å­Eanti-spamEˆãƒ¬ãƒ¼ãƒˆåˆ¶é™ãEæŠ•ç¥¨å¤‰æ›´å›æ•°E‰ã ã‘ã§ã¯ã€EEä¿¡ç¾å ´ã§ã®æ˜ç¤ºçšEªé™¤å¤–åˆ¶å¾¡ãŒè¶³ã‚Šãªã‹ã£ãŸã€E
### What
- `apps/web/src/lib/stream_moderation.ts` ã‚’æ–°è¦è¿½åŠ ã€E  - BAN åˆ¤å®šã€NGãƒ¯ãƒ¼ãƒ‰åˆ¤å®šã€slow mode åˆ¤å®šã‚’ pure function åŒ–ã€E  - comma/newline å½¢å¼ãEè¨­å®šæ–‡å­—åEã‚’æ­£è¦åŒ–ãƒ»é‡è¤E™¤å»ã™ã‚‹ãƒ‘ãEã‚µã‚’è¿½åŠ ã€E- `apps/web/src/pages/Stream.tsx`
  - moderation è¨­å®Estate ã‚’è¿½åŠ EElow mode ç§’æ•° / banned users / blocked wordsE‰ã€E  - localStorage æ°¸ç¶šåŒ–ã‚’è¿½åŠ EEstream.moderation.*`E‰ã€E  - `addVoteFromChat` ã§å—ç†å‰ã« moderation åˆ¤å®šã‚’é©ç”¨:
    - banned user reject
    - blocked word reject
    - slow mode reject
  - vote audit ã« `banned/ng-word/slow` ã® reject ã‚«ã‚¦ãƒ³ã‚¿ã‚’è¿½åŠ ã€E- `apps/web/src/components/stream/VoteControlPanel.tsx`
  - Moderation UIEElow modeç§’æ•°ãƒ»BAN listãƒ»NG wordsE‰ã‚’è¿½åŠ ã€E  - audit è¡¨ç¤ºã« moderation reject å†E¨³ã‚’è¿½åŠ ã€E- `apps/web/src/lib/local_settings.ts`
  - moderation è¨­å®šãE read/write ãƒ˜ãƒ«ãƒ‘ã‚’è¿½åŠ ã€E- Tests:
  ...
  - `docs/00_handoff/Nyano_Triad_League_LONG_TERM_ROADMAP_v1_ja.md` ã® Phase 4 moderation é E›®ã‚’å®ŒäºE«æ›´æ–°ã€E  - `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md` ã« Commit0108 ã‚’è¿½è¨˜ã€E
### Verify
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web lint`
- `pnpm -C apps/web build`
...
### Why
- DEV_TODO ã®é«˜å„ªå…ˆé E›®ã€Œãƒ©ãƒ€ãƒ¼Eˆãƒ©ãƒ³ã‚­ãƒ³ã‚°E‰ã‚’è¨±å¯ä¸è¦ã§ç¬¬ä¸‰è€E‹ç”¨ã§ãã‚‹ãƒ•ã‚©ãƒ¼ãƒãƒƒãƒˆã€ãŒæœªå®ŒäºE ã£ãŸã€E- transcript / settled event / ç½²åãE3ç‚¹ã‚’æœ€å°ã‚»ãƒEƒˆã§å›ºå®šã—ãªãE¨ã€åŒã˜ãƒ‡ãƒ¼ã‚¿ã§ã‚‚åEè¨ˆç®—çµæœãŒæºã‚Œã‚‹ãƒªã‚¹ã‚¯ãŒã‚ã£ãŸã€E- indexerä¾å­˜ã‚’é¿ã‘ã‚‹ãŸã‚ã€E‡è¤EEçEEã‚½ãƒ¼ãƒˆé EEtie-breaké E‚’ä»•æ§˜ã¨ã—ã¦å›ºå®šã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `packages/triad-engine/src/ladder.ts` ã‚’æ–°è¦è¿½åŠ ã€E  - `LadderMatchAttestationV1`EEIP-712E‰ã‚’è¿½åŠ ã€E    - typed-data payload / digest / signer recover / signature verify ã‚’å®Ÿè£E€E  - `LadderMatchRecordV1` æ¤œè¨¼ã‚’å®Ÿè£E€E    - `hashTranscriptV1(transcript) == settled.matchId` ã‚’å¿E ˆåŒ–ã€E    - transcript header ã¨ settled event ã® ruleset/season/player ä¸€è‡´ã‚’æ¤œè¨¼ã€E    - playerA/playerB ã®ä¸¡ç½²åæ¤œè¨¼ã‚’å¿E ˆåŒ–ã€E  - `buildLadderStandingsV1(...)` ã‚’å®Ÿè£E€E    - sourceã‚­ãƒ¼EEhainId:blockNumber:txHash:logIndexE‰ã§é‡è¤E’é™¤ã€E    - åŒä¸€sourceã®å†E®¹ä¸ä¸€è‡´ã‚Erejectã€E    - points / wins / draws / losses / tileDiff ã‚’é›†è¨ˆã€E    - tie-breaké E‚’å›ºå®šï¼Eoints desc â†Ewins desc â†EtileDiff desc â†Elosses asc â†Eaddress ascE‰ã€E- `packages/triad-engine/src/index.ts`
  - `ladder` ã‚¨ã‚¯ã‚¹ãƒãEãƒˆã‚’è¿½åŠ ã€E- `packages/triad-engine/test/ladder.test.js`
  - æ­£å¸¸ç³»ã€transcriptä¸ä¸€è‡´ã€ç½²åä¸ä¸€è‡´ã€E‡è¤E’é™¤ã€conflicting duplicate rejectionã€å›ºå®štie-breakã‚’è¿½åŠ æ¤œè¨¼ã€E- `docs/02_protocol/Nyano_Triad_League_LADDER_FORMAT_SPEC_v1_ja.md`
  - ladder v1 ã®ãƒ•ã‚©ãƒ¼ãƒãƒƒãƒˆä»•æ§˜ã‚’æ–°è¦è¿½åŠ ã€E- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md`
  - ladderé E›®ã‚’å®ŒäºE«æ›´æ–°ã€E
### Verify
- `pnpm -C packages/triad-engine lint`
- `pnpm -C packages/triad-engine build`
- `pnpm -C packages/triad-engine test`Eˆã“ã®å®Ÿè¡Œç’°å¢E§ã¯ `node:test` ãE`spawn EPERM` ã®ãŸã‚å®Œèµ°ä¸å¯EE- `node -e ...` ã§ ladder ã®ç½²åæ¤œè¨¼ãƒ»standingsé›E¨ˆã‚’ã‚¹ãƒ¢ãƒ¼ã‚¯å®Ÿè¡Œï¼ˆæEåŠŸï¼E
## 2026-02-12 - commit-0106: phase3 hardening (web error tracking + release runbook)

### Why
- Phase 3 ã®æœªå®ŒäºE E›®Eˆã‚¨ãƒ©ãƒ¼ãƒˆãƒ©ãƒE‚­ãƒ³ã‚° / ãƒªãƒªãƒ¼ã‚¹æ‰‹é E¼‰ãŒæ®‹ã£ã¦ãŠã‚Šã€å›å¸°æ¤œçŸ¥ã¨å‡ºè·æ‰‹é EEæ¨™æº–åŒ–ãŒä¸è¶³ã—ã¦ãEŸã€E- ä¾å­˜è¿½åŠ ã‚’æœ€å°ã«æŠ‘ãˆã¤ã¤ã€ã¾ãšå®Ÿé‹ç”¨ã§ãã‚‹ã‚¨ãƒ©ãƒ¼åé›†ã®åŸºç›¤ãŒå¿E¦ã ã£ãŸã€E
### What
- `apps/web/src/lib/error_tracking.ts` ã‚’æ–°è¦è¿½åŠ ã€E  - global `error` / `unhandledrejection` å‘ã‘ã®åé›†ãƒ­ã‚¸ãƒE‚¯ã‚’å®Ÿè£E€E  - sink ã‚’åEæ›¿å¯èƒ½åŒ–ï¼Elocal` / `console` / `remote`E‰ã€E  - localStorage ãƒªãƒ³ã‚°ãƒãƒƒãƒ•ã‚¡Eˆæ—¢å®E0ä»¶E‰ã§å±¥æ­´ä¿æŒã€E  - env è¨­å®E
    - `VITE_ERROR_TRACKING_MODE`
    - `VITE_ERROR_TRACKING_ENDPOINT`
    - `VITE_ERROR_TRACKING_MAX_EVENTS`
    - `VITE_APP_RELEASE`
- `apps/web/src/main.tsx`
  - `installGlobalErrorTracking()` ã‚’èµ·å‹•æ™‚ã«å°åEã€E- `apps/web/src/lib/__tests__/error_tracking.test.ts`
  - sinkè§£æã€ã‚¤ãƒ™ãƒ³ãƒˆæ­£è¦åŒ–ã€ãƒ­ãƒ¼ã‚«ãƒ«ä¿æŒã€ã‚¯ãƒªã‚¢ã€console sink ã‚’æ¤œè¨¼ã€E- `package.json`
  - `release:check` ã‚¹ã‚¯ãƒªãƒ—ãƒˆã‚’è¿½åŠ EEngine lint/build + web typecheck/lint/buildE‰ã€E- `docs/99_dev/RELEASE_RUNBOOK_v1_ja.md`
  ...
  - Phase 3 ã®æœªå®ŒäºEé E›®ã‚’å®ŒäºE«æ›´æ–°ã€E- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md`
  - Commit0106 ã‚’åæ˜ ã—ã€Doing ã‚’æ¬¡ãƒ•ã‚§ãƒ¼ã‚ºã¸æ›´æ–°ã€E
### Verify
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web lint`
...
### Why
- `resolveFirstPlayerV1` ã‚’å°åEã—ãŸå¾Œã‚‚ã€ä¸¡è€Eˆæ„ãƒ•ãƒ­ãƒ¼ã®ã€Œcommitæ¤œè¨¼ä»˜ãã€å°ç·šãŒä¸è¶³ã—ã¦ãEŸã€E- web å´ã® first-player UI ã¯ `manual / mutual / commit_reveal` ã®3ãƒ¢ãƒ¼ãƒ‰ãEã¿ã§ã€seed ãƒ•ãƒ­ãƒ¼ã‚’ç›´æ¥æ¤œè¨¼ã§ããªã‹ã£ãŸã€E
### What
- `packages/triad-engine/src/first_player.ts`
...
### Why
- Engineå´ã§ `committed_mutual_choice` ã‚’è¿½åŠ æ¸ˆã¿ã ã£ãŸãŒã€web Match UI ã‹ã‚‰ã¯é¸æŠãEæ¤œè¨¼ã§ããªã‹ã£ãŸã€E- ã€ŒåEå¹³ãªå…ˆæ”»æ±ºå®šï¼Eommitä»˜ãä¸¡è€Eˆæ„ï¼‰ã€ã‚’å®Ÿé‹ç”¨ã§è©¦ã™ã«ã¯ã€URLãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã¨UIã®ä¸¡æ–¹ã§å†ç¾å¯èƒ½ã«ã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/lib/first_player_resolve.ts`
...
### Why
- web å´ `first_player_resolve` ãEengine ã®åˆ¤å®šãƒ­ã‚¸ãƒE‚¯ã‚’éƒ¨åˆEš„ã«å†å®Ÿè£E—ã¦ãŠã‚Šã€å°E¥ãƒ¢ãƒ¼ãƒ‰è¿½åŠ æ™‚ã«ä¹–é›¢ãƒªã‚¹ã‚¯ãŒã‚ã£ãŸã€E- `commit_reveal` ã§ç‰EEcommitã ã‘ã‚’å—ã‘å…¥ã‚Œã‚‹ä½™åœ°ãŒæ®‹ã£ã¦ãEŸãŸã‚ã€engineå´ãƒãƒªã‚·ãƒ¼ã¨æEˆã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/lib/first_player_resolve.ts`
  ...
  - `commit_reveal` ã®commitå…¥åŠ›ã‚’å³å¯EŒ–:
    - commitA/commitB ã©ã¡ã‚‰ã‹ç‰E–¹ã®ã¿ã¯ã‚¨ãƒ©ãƒ¼ã€E    - ä¸¡æ–¹å…¥åŠ›æ™‚ã®ã¿ engine resolver ã¸ commit pair ã‚’æ¸¡ã™ã€E  - æ—¢å­˜ãE UI å‘ã‘ã‚¨ãƒ©ãƒ¼ãƒãƒ³ãƒ‰ãƒªãƒ³ã‚°EEanual fallback + erroræ–E­—åEE‰ãEç¶­æŒã€E- `apps/web/src/lib/__tests__/first_player_resolve.test.ts`
  - `commit_reveal` ã®ä¸ä¸€è‡´ãƒE‚¹ãƒˆã‚’ã€Œä¸¡å´commitå…¥åŠ›ã‚ã‚Šã€ãEå½¢ã«æ›´æ–°ã€E  - ç‰EEcommitå…¥åŠ›ã‚’æ˜ç¤ºçšE« reject ã™ã‚‹ãƒE‚¹ãƒˆã‚’è¿½åŠ ã€E
### Verify
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web lint`
- `pnpm -C apps/web test -- src/lib/__tests__/first_player_resolve.test.ts`
...
### Why
- `Match.tsx` ã® first-player è¨­å®šãEã‚¿ãƒ³ã§ `setParam(...)` ã‚’é€£ç¶šå‘¼ã³å‡ºã—ã—ã¦ãŠã‚Šã€URLãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿æ›´æ–°ãŒå–ã‚Šã“ã¼ã‚Œã‚‹å¯èƒ½æ€§ãŒã‚ã£ãŸã€E- `commit_reveal` / `committed_mutual_choice` ã®å…¥åŠ›æ¡ä»¶ãŒUIä¸Šã§ä¼ã‚ã‚Šã¥ã‚‰ãã€èª¤å…¥åŠ›æ™‚ã®æ‰‹æˆ»ã‚ŠãŒç™ºç”Ÿã—ã‚E™ã‹ã£ãŸã€E
### What
- `apps/web/src/pages/Match.tsx`
...
### Why
- ãƒ¢ãƒ¼ãƒ‰åEæ›¿æ™‚ã«ä¸è¦ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã¯æ¶ˆãˆã‚‹ã‚ˆãE«ãªã£ãŸãŒã€å¿E ˆåEåŠ›ãŒç©ºã®ã¾ã¾æ®‹ã‚‹ã‚±ãƒ¼ã‚¹ãŒã‚ã‚Šã€åEæ›¿ç›´å¾Œã« invalid ã«ãªã‚Šã‚„ã™ã‹ã£ãŸã€E- `seed / commit_reveal / committed_mutual_choice` ã§ã¯ã€åEå›åEåŠ›ã‚³ã‚¹ãƒˆã¨å…¥åŠ›ãƒŸã‚¹ã‚’æ¸›ã‚‰ã™ãŸã‚ã«å®‰åEãªåˆæœŸå€¤è£œå®ŒãŒå¿E¦ã ã£ãŸã€E
### What
- `apps/web/src/lib/first_player_params.ts`
  ...
  - Mode switch default-fill behavior:
    ...
    - `seed`: `fps/fpsd` ãEbytes32 ã§ãªã‘ã‚Œã°è‡ªå‹•è£œå®E    - `commit_reveal`: `fps/fra/frb` ã‚’èEå‹•è£œå®Œã—ã€`fca/fcb` ã¯ã‚¯ãƒªã‚¢
...
### Why
- first-player ãƒ¢ãƒ¼ãƒ‰åEæ›¿ã¯ URL ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿çŠ¶æ…‹ã«å¼·ãä¾å­˜ã™ã‚‹ãŸã‚ã€ãƒ¦ãƒ‹ãƒƒãƒˆãƒ†ã‚¹ãƒˆã ã‘ã§ã¯ç”»é¢å®Ÿå‹•ä½œãEå›å¸°ã‚’æ‹¾ãEã‚ŒãªãE€E- mode transition æ™‚ãEã€Œä¸è¦å€¤ã‚¯ãƒªã‚¢ + å¿E ˆå€¤è£œå®Œã€ãŒå´©ã‚Œã‚‹ã¨ã€åEæœ‰URLå†ç¾æ€§ã¨UXãŒæ‚ªåŒ–ã™ã‚‹ã€E
### What
- `apps/web/e2e/match-first-player.spec.ts` ã‚’æ–°è¦è¿½åŠ ã€E  - Case 1: `manual` â†E`commit_reveal`
    - `fps/fra/frb` ãEbytes32 ã§åŸ‹ã¾ã‚‹ã“ã¨
    - `fpsd` / committed-mutualç³»ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ãŒã‚¯ãƒªã‚¢ã•ã‚Œã‚‹ã“ã¨
  - Case 2: `commit_reveal` â†E`committed_mutual_choice`
    - `fps/fpna/fpnb` ãEbytes32 ã§åŸ‹ã¾ã‚‹ã“ã¨
    - `fpa/fpb` ãŒæ­£è¦åŒ–ã•ã‚Œã‚‹ã“ã¨
    - `fpoa/fpob` ãŒæ—¢å®šå€¤ã§è£œå®Œã•ã‚Œã‚‹ã“ã¨
    - `fra/frb/fca/fcb/fpsd` ãŒã‚¯ãƒªã‚¢ã•ã‚Œã‚‹ã“ã¨
...
### Why
- `NyanoCardArt` ã¯ gateway fallback ã‚’è©¦ã—ãŸå¾Œã«å³ placeholder å›ºå®šã¨ãªã‚Šã€å›ç·šå¾©å¸°æ™‚ã«ãƒ¦ãƒ¼ã‚¶ãƒ¼ãŒåEè©¦è¡Œã§ããªã‹ã£ãŸã€E- åŒã˜ URL ã¸ã®å†èª­è¾¼ã§ã¯ãƒ–ãƒ©ã‚¦ã‚¶ã‚­ãƒ£ãƒE‚·ãƒ¥ã«ã‚ˆã‚Šå¤±æ•—çŠ¶æ…‹ãŒæ®‹ã‚‹ã‚±ãƒ¼ã‚¹ãŒã‚ã‚Šã€æEç¤ºçšEª cache-busting ãŒå¿E¦ã ã£ãŸã€E
### What
...
### Why
- DEV_TODO ã®é«˜å„ªå…ˆé E›®ã¨ã—ã¦ã€Œã‚·ãƒ¼ã‚ºãƒ³ã®è­°ä¼šï¼Euleset proposal / vote / adoptE‰ã€ãŒæœªå®ŒäºE ã£ãŸã€E- é‹å–¶ä¸åœ¨ã§ã‚‚ç¬¬ä¸‰è€EŒåŒã˜æ¡æŠçµæœã‚’åEç¾ã§ãã‚‹ã‚ˆã†ã«ã€æ±ºå®šè«–ãªé›E¨ˆè¦å‰‡ã‚’åEã«å›ºå®šã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E- ç½²åæŠ•ç¥¨EEIP-712E‰ã‚’å°åEã™ã‚‹å‰æã‚’å´©ã•ãªãE½¢ã§ã€æœ€å°ãE TS å‚çEå®Ÿè£E‚’è¿½åŠ ã—ãŸã‹ã£ãŸã€E
### What
- `packages/triad-engine/src/season_council.ts` ã‚’æ–°è¦è¿½åŠ ã€E  - Proposal:
  ...
  - Tally/Adopt:
    - `tallySeasonCouncilVotesV1(...)`
      - åŒä¸€ voter ã¯æœ€å¤§ nonce æ¡ç”¨
      - åŒä¸€ nonce ç«¶åˆãEã‚¨ãƒ©ãƒ¼
      - proposalä¸ä¸€è‡´ / æœŸé™åˆE‚Œ / å€™è£œå¤–ãE reject
      - åŒç‡ã¯ `rulesetId` æ˜E E§ tie-break
    - `adoptSeasonCouncilRulesetV1(...)`
      - quorum åˆ°éE+ winner å­˜åœ¨æ™‚ãEã¿æ¡æŠE- `packages/triad-engine/src/index.ts`
  - `season_council` ã‚¨ã‚¯ã‚¹ãƒãEãƒˆã‚’è¿½åŠ ã€E- `packages/triad-engine/test/season_council.test.js`
  - proposalId canonicalizationã€vote hash æ±ºå®šæ€§ã€EIP-712 sign/verify/recoverã€nonce ç«¶åˆã€tally/adopt æ¡ä»¶ã‚’è¿½åŠ æ¤œè¨¼ã€E- `docs/02_protocol/Nyano_Triad_League_SEASON_COUNCIL_SPEC_v1_ja.md`
  - v1 æœ€å°ãEãƒ­ãƒˆã‚³ãƒ«ä»•æ§˜ã‚’æ–°è¦è¿½åŠ EEroposal/vote/adoptã€deterministic ruleã€EIP-712 å‹ï¼‰ã€E- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md`
  - Windå…¬å¹³åŒ–ã‚’å®ŒäºE«æ›´æ–°ã€E  - ã€Œã‚·ãƒ¼ã‚ºãƒ³ã®è­°ä¼šã€é E›®ã‚’å®ŒäºE«æ›´æ–°ã€E  - Doing ã‚’ã€Œãƒ©ãƒ€ãƒ¼ format å›ºå®šã€ã¸æ›´æ–°ã€E
### Verify
- `pnpm -C packages/triad-engine lint`
- `pnpm -C packages/triad-engine test`
...
### Verify
- `pnpm.cmd lint` âœE- `pnpm.cmd test` âœE- `pnpm.cmd -C packages/triad-engine build` âœE- `pnpm.cmd -C packages/triad-engine lint` âœE- `node packages/triad-engine/test/classic_ruleset_id_v2.test.js` âœE- `node packages/triad-engine/test/classic_order_chaos_swap.test.js` âœE- `node packages/triad-engine/test/classic_reverse_ace.test.js` âœE- `node packages/triad-engine/test/classic_plus_same.test.js` âœE- `node packages/triad-engine/test/classic_type_ascend_descend.test.js` âœE- `pnpm.cmd -C apps/web build` âœE- `pnpm.cmd -C apps/web typecheck` âE(env issue: TS cannot resolve `pixi.js` / `fflate` in this sandbox run)
- `pnpm.cmd -C apps/web test -- ...` âE(sandbox `spawn EPERM` while loading vite/esbuild)
- `pnpm.cmd build:web` âE(sandbox `spawn EPERM` in nested pnpm/vite invocation)
...
- Rebuilt `apps/web/src/pages/Rulesets.tsx` with:
  - Recommended section (`ãŠã™ã™ã‚`) using top curated presets.
  - One-line summary surfaced for each ruleset row.
  - Direct CTA `ã“ãEãƒ«ãƒ¼ãƒ«ã§å¯¾æˆ¦` linking to `/match?ui=mint&rk=<rulesetKey>`.
...
### Why
- WO009 UI was implemented, but discovery flow regressions (`ãŠã™ã™ã‚` visibility and `/match` CTA routing) were not guarded by browser E2E.
...
### Why
- å‚çEç”»åƒãEãƒ¼ã‚¹ã®UIå¼·åŒ–ã§ã€ä¸Šéƒ¨ã®çŠ¶æ³æŠŠæ¡Eˆã‚¹ã‚³ã‚¢/ã‚¿ãƒ¼ãƒ³E‰ã‚’å®šä½ç½®åŒ–ã—ã€ç›¤é¢ã¸ã®è¦–ç·šé›†ä¸­ã‚’æ”¹å–E™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E- æ—¢å­E`BattleHudMint` ã¯æƒE ±é‡ãŒå¤šãã€`density=minimal` ã§ã¯ã‚ˆã‚Šã‚·ãƒ³ãƒ—ãƒ«ãªå›ºå®šHUDãŒå¿E¦ã ã£ãŸã€E
### What
...
### Why
- å‚çEç”»åƒã«ã‚ã‚‹ã€Œå·¦å³ãƒ—ãƒ¬ã‚¤ãƒ¤ãƒ¼ãƒ‘ãƒãƒ« + ç›¤é¢ä¸­å¿E€ãEå¯¾æˆ¦ãƒ¬ã‚¤ã‚¢ã‚¦ãƒˆã‚’å°åEã—ã€å¯¾æˆ¦ã®å­˜åœ¨æ„Ÿã‚’é«˜ã‚ã‚‹å¿E¦ãŒã‚ã£ãŸã€E- æ—¢å­˜ãEç›¤é¢å…¥åŠ›ç³»ã‚’å´©ã•ãšã€è¦‹ãŸç›®ãƒ¬ã‚¤ãƒ¤ãƒ¼ã¨ã—ã¦è¿½åŠ ã™ã‚‹æ–¹é‡ãŒå¿E¦ã ã£ãŸã€E
### What
...
### Why
- æ“ä½œãEèµ·ç‚¹Eˆæ‰‹æœ­é¸æŠï¼‰ã¨æ¬¡ã‚¢ã‚¯ã‚·ãƒ§ãƒ³èª˜å°ï¼EromptE‰ãEã€å‚ç…§ç”»åƒãEãƒ¼ã‚¹UIã®ä½“é¨“å“è³ªã«ç›´çµã™ã‚‹ã€E- æ—¢å­˜UIã§ã¯æ‰‹æœ­ãŒå˜ç´”ãªè¡Œè¡¨ç¤ºã§ã€prompt ã‚Eæ®µè¡¨ç¤ºã ã£ãŸãŸã‚ã€ã‚²ãƒ¼ãƒ ç”»é¢ã¨ã—ã¦ã®â€œå®šä½ç½®æ„Ÿâ€ã‚’å¼·åŒ–ã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
...
### Why
- Nyano ã‚³ãƒ¡ãƒ³ãƒˆè¡¨ç¤ºæ™‚ã« slot é«˜ã•ã‚E–‡è¨€å·®åˆE§ç¸¦æ–¹å‘ãEæºã‚ŒãŒæ®‹ã‚‹å¯èƒ½æ€§ãŒã‚ã‚Šã€Mint match ã®å®‰å®šæ„Ÿã‚’æãªã£ã¦ãEŸã€E- æ—¢å­˜ãEãƒ¬ã‚¤ã‚¢ã‚¦ãƒˆå®‰å®šåŒ–ã‚’è£œå¼·ã—ã€`input` ã¯ã‚ã‚‹ãE`kind=idle` ã§å®Ÿè¡¨ç¤ºãŒãªãE‚±ãƒ¼ã‚¹ã‚‚å®‰åEã«æ‰±ãE¿E¦ãŒã‚ã£ãŸã€E
### What
- Updated `apps/web/src/components/NyanoReactionSlot.tsx`:
  - `pickReactionKind` ã‚’ä½¿ã£ã¦ `hasVisibleReaction` ã‚’åˆ¤å®šã€E  - slot å†EEå¸¸æ™Eplaceholder ã‚’ãEã‚¦ãƒ³ãƒˆã—ã€`mint-nyano-reaction-slot__content` ã« reaction ã‚’é‡ã­ã‚‹æ§‹é€ ã¸å¤‰æ›´ã€E  - `input !== null` ã§ã‚E`kind=idle` ã®å ´åˆãE idle slot class ã‚’ç¶­æŒã€E- Updated `apps/web/src/mint-theme/mint-theme.css`:
  - slot ã‚E`min-height` ä¾å­˜ã‹ã‚E`height: clamp(...)` + `overflow: hidden` ã«å¤‰æ›´ã€E  - `mint-nyano-reaction-slot__content` ã‚Eabsolute overlay åŒ–ã—ã€reaction æœ¬ä½“ã‚’ `inset: 0` ã§å›ºå®šã€E  - `stage-focus-cutin` ã®ä½™ç™½ã‚’é™¤å»ã—ã¦ slot å†Eã¾ã‚Šã‚’å®‰å®šåŒ–ã€E- Updated `apps/web/src/components/__tests__/NyanoReactionSlot.test.tsx`:
  - placeholder + content wrapper æ§‹é€ ã«åˆã‚ã›ã¦ãƒE‚¹ãƒˆæ›´æ–°ã€E  - `kind=idle` ã‚±ãƒ¼ã‚¹ã§ slot ãEidle æ‰±ãE«ãªã‚‹ã“ã¨ã‚’è¿½åŠ æ¤œè¨¼ã€E- Updated `apps/web/e2e/ux-guardrails.spec.ts`:
  - LayoutShift API ã®è»½é‡ãEãƒ­ãƒ¼ãƒ–ã‚’è¿½åŠ ã€E  - Nyano slot ã‚·ãƒŠãƒªã‚ªã§ line-clamp/overflow ã¨åˆã‚ã›ã¦ layout-shift ã—ããE€¤ãƒã‚§ãƒE‚¯ã‚’è¿½åŠ ã€E
### Verify
- `pnpm.cmd -C apps/web test -- NyanoReactionSlot` OK
- `pnpm.cmd -C apps/web e2e -- e2e/ux-guardrails.spec.ts` OK
- `pnpm.cmd -C apps/web build` OK
- `pnpm.cmd -C apps/web typecheck` NGEˆæ—¢å­˜ä¾å­˜ä¸è¶³: `pixi.js` / `fflate` å‹è§£æ±ºã‚¨ãƒ©ãƒ¼EE
## 2026-02-15 - WO016: Mint microinteraction polish (press/hover/focus unification)

### Why
- Mint UI å†E§ã‚»ãƒ«ãƒ»æ‰‹æœ­ãƒ»ãƒœã‚¿ãƒ³ã®æŠ¼ä¸‹æ–‡æ³•ãŒå ´æ‰€ã”ã¨ã«å¾®å¦™ã«ç•°ãªã‚Šã€â€œè§¦ã‚Šå¿Eœ°â€Eã®çµ±ä¸€æ„ŸãŒä¸è¶³ã—ã¦ãEŸã€E- ã‚­ãƒ¼ãƒœãEãƒ‰æ“ä½œæ™‚ã®è¦–èªæ€§EEocus-visibleE‰ã‚‚æEˆã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- Updated `apps/web/src/mint-theme/mint-theme.css`:
  - å…±é€šãƒ¦ãƒ¼ãƒE‚£ãƒªãƒE‚£ `mint-pressable` / `mint-pressable--cell|--card|--pill` ã‚’è¿½åŠ ã€E  - hover/active/focus-visible ã‚ECSS å¤‰æ•° `--mint-press-*` ã§çµ±ä¸€ã€E  - selected ring/glow ã‚E`--mint-selected-ring` / `--mint-selected-glow` ã«çµ±ä¸€ã—ã€A/B è‰²ã¨ä¸¡ç«‹ã€E  - `prefers-reduced-motion` ã¨ `data-vfx=off|low` ã« pressæ¼”åEæŠ‘åˆ¶ã‚’è¿½åŠ ã€E- Updated `apps/web/src/components/BoardViewMint.tsx`:
  - selectable cell ã« `mint-pressable mint-pressable--cell` ã‚’ä»˜ä¸ã€E  - `tabIndex=0` ã¨ Enter/Space ã§ã®ã‚»ãƒ«é¸æŠã‚’è¿½åŠ EEocus-visible å°ç·šï¼‰ã€E- Updated `apps/web/src/components/HandDisplayMint.tsx`:
...
### Why
- WO016 ã§è¿½åŠ ã—ãŸ `mint-pressable` ã¨ Enter/Space å°ç·šãEã€è¦‹ãŸç›®ã‚ˆã‚Šå…ˆã«æ“ä½œæ„ŸãŒå£Šã‚Œã‚E™ãE€E- æ—¢å­E`ux-guardrails` ã¯ URLåŒæœŸã¨ãƒ¬ã‚¤ã‚¢ã‚¦ãƒˆå®‰å®šãŒä¸­å¿E§ã€ã‚­ãƒ¼ãƒœãEãƒ‰å°ç·šã¨ reduced-motion æŠ‘åˆ¶ã®å›å¸°ã‚’æ•ã¾ãˆã‚‰ã‚Œã¦ãEªã‹ã£ãŸã€E
### What
- Updated `apps/web/e2e/ux-guardrails.spec.ts`:
  - Added test: `Mint board cells remain keyboard-selectable via Enter`
    - Mint match ã§ hand card é¸æŠå¾Œã€`data-board-cell` ã¸ãƒ•ã‚©ãƒ¼ã‚«ã‚¹ã—ã¦ Enter ã§ `mint-cell--selected` ã«ãªã‚‹ã“ã¨ã‚’æ¤œè¨¼ã€E  - Added test: `Reduced motion disables pressable transition feedback in Mint battle UI`
    - `page.emulateMedia({ reducedMotion: "reduce" })` ä¸‹ã§ hand card / board cell ã® `transitionDuration` ãE`0s` ã‚’å«ã‚€ã“ã¨ã‚’æ¤œè¨¼ã€E
### Verify
...
### Why
- ãƒãƒˆãƒ«ä¸­ã€Nyanoã‚³ãƒ¡ãƒ³ãƒˆã«åŒæœŸã—ã¦ç›¤é¢ãŒã€Œãšã‚Œã‚‹ã€ä½“æ„ŸãŒã‚ã‚Šã€æ“ä½œãEå®‰å®šæ„Ÿã‚’æãªã£ã¦ãEŸã€E- ã‚ã‚ã›ã¦ ActionPromptEˆã€Œã‚«ãƒ¼ãƒ‰ã‚’é¸ã‚“ã§ãã ã•ã„ã€ï¼‰ãŒå¤§ãã™ãã€ç›¤é¢è¦–èªæ€§ã‚’é˜»å®³ã—ã¦ãEŸã€E
### What
- Updated `apps/web/src/mint-theme/mint-theme.css`:
  - `mint-stage-impact-board` ã® transformç³»ã‚¢ãƒ‹ãƒ¡ãƒ¼ã‚·ãƒ§ãƒ³ã‚’å»E­¢ã—ã€ä½ç½®ã‚’å‹•ã‹ã•ãªãE`mint-stage-impact-board-glow` ã¸ç½®æ›ã€E  - `mint-nyano-reaction-slot` ã« `contain: layout paint` ã‚’è¿½åŠ ã—ã€ã‚³ãƒ¡ãƒ³ãƒˆæç”»ã®ãƒ¬ã‚¤ã‚¢ã‚¦ãƒˆå½±éŸ¿ã‚’å±€æ‰€åŒ–ã€E  - `mint-prompt__ja` / `mint-prompt__en` ã®ãƒ•ã‚©ãƒ³ãƒˆã‚µã‚¤ã‚ºã‚’ç¸®å°ï¼Eesktop/mobileä¸¡æ–¹E‰ã€E- Updated `apps/web/e2e/ux-guardrails.spec.ts`:
  - Nyano slotã‚·ãƒŠãƒªã‚ªã§ board frame ã® documentåº§æ¨™å·®åˆEƒã‚§ãƒE‚¯ã‚’è¿½åŠ EEeck PreviewçŠ¶æ…‹ã‚’å›ºå®šã—ã¦è¨ˆæ¸¬E‰ã€E
### Verify
...
### Why
- `/match` ˆÈŠOiHome/Arena/Decks/Onboardingj‚ªŠÇ—‰æ–ÊŠñ‚è‚ÅAQÆMock‚ÌuƒXƒ}ƒzƒQ[ƒ€UIv‚©‚ç˜¨—£‚µ‚Ä‚¢‚½B
- ‰æ–Ê‰¡“WŠJ‚Ì‚½‚ß‚ÉAMint UI ƒvƒŠƒ~ƒeƒBƒuiGlass/Pressable/Icon/TabNav/BigButtonj‚ğ‹¤’Ê‰»‚·‚é•K—v‚ª‚ ‚Á‚½B
- «—ˆ‚Ì‰æ‘œ·‚µ‘Ö‚¦‚É”õ‚¦‚ÄAGemini ¶¬ƒpƒCƒvƒ‰ƒCƒ“‚Æå—v‰æ–Ê‚Ì e2e ƒK[ƒh‚ğÄŠm”FE®”õ‚µ‚½‚©‚Á‚½B
...
- Updated app layout:
  - `apps/web/src/App.tsx`
    - Mint theme ‚Ì‚İ App chrome ‚ğ `MintGameShell + MintAppChrome` ‚ÖØ‘ÖB
    - `focusRoute`istage/focusj‚Å‚Í]—ˆ‚Ç‚¨‚è header/footer ”ñ•\¦‚ğˆÛB
    - `prefers-reduced-motion` / `data-vfx` ‚Ö‚ÌŠù‘¶§Œä‚ÍˆÛB
- Rebuilt main screens with Mint structure:
  - `apps/web/src/pages/Home.tsx`i4‘åƒ{ƒ^ƒ“A3ƒXƒeƒbƒvAinfobarATools/Settingsj
  - `apps/web/src/pages/Arena.tsx`iside nav + banner + quick play + difficulty cardsj
  - `apps/web/src/pages/Decks.tsx`i3ƒJƒ‰ƒ€ Deck Builderj
  - `apps/web/src/pages/Start.tsx`ionboarding 3 cards + progress pillj
  - `apps/web/src/main.tsx`i`/start` route’Ç‰Áj
  - `apps/web/src/components/CardBrowser.tsx`ipreset filter props’Ç‰Áj
- Expanded Mint CSS for app screens/primitives:
  - `apps/web/src/mint-theme/mint-theme.css`
    - Shell”wŒiAglassƒpƒlƒ‹Atab/button/typographyAHome/Arena/Decks/Start ƒŒƒCƒAƒEƒgŒQ‚ğ’Ç‰ÁB
    - reduced-motion ‚Æ `data-vfx=off` ‚Å”wŒi‰‰o‚ğ—}§B
- Gemini pipeline availability + asset directory:
  - `apps/web/public/assets/gen/.gitkeep` ’Ç‰Ái¶¬•¨’u‚«ê‚ğŒÅ’èj
  - Existing `scripts/gemini_image_gen.mjs` / batch prompt / docs ‚Ì‰^—p‚ğŠm”FB
- e2e guardrails:
...
### Why
- `focusRoute` (`focus=1`, `/battle-stage`, `/replay-stage`) ã§ã¯ App chrome ã‚’åEã•ãªãE‰æãŒã‚ã‚Šã€Mintç”»é¢æ‹¡å¼µå¾Œã‚‚ã“ãEäº’æ›ã‚’å›ºå®šã—ã¦ãŠãå¿E¦ãŒã‚ã‚‹ã€E- Mint tab navigation æ™‚ãE `theme=mint` å¼•ãå›ã—EERLäº’æ›E‰ã‚‚ e2e ã§ç›´æ¥æ‹E¿ã—ãŸã„ã€E
### What
- Updated `apps/web/e2e/mint-app-screens-guardrails.spec.ts`:
  - Added `Mint app chrome preserves theme query across tab navigation`
    - `/?theme=mint` ã‹ã‚‰ Arena/Decks ã‚¿ãƒ–éEç§»æ™‚ã« `theme=mint` ã‚’ä¿æŒã™ã‚‹ã“ã¨ã‚’ç¢ºèªã€E  - Added `focus routes keep app chrome hidden for layout compatibility`
    - `/match?...&focus=1` ã¨ `/battle-stage?...&focus=1` ã§
      `.mint-app-chrome` / `.mint-app-footer` / `.app-header` / `.app-footer` ãŒéè¡¨ç¤ºã§ã‚ã‚‹ã“ã¨ã‚’ç¢ºèªã€E    - `/battle-stage` ã§ã¯ `Commit move from focus hand dock` å¯è¦–ã‚‚ç¢ºèªã—ã¦ã€focuså‹•ç·šãŒç¶­æŒã•ã‚Œã‚‹ã“ã¨ã‚’æ¤œè¨¼ã€E
### Verify
- `pnpm -C apps/web test` OK
- `pnpm -C apps/web typecheck` OK
- `pnpm -C apps/web build` OK
...
### Why
- `Arena` ã§è¿½åŠ ã•ã‚ŒãE`MintPageGuide` ã¨é›£æ˜“åº¦ãƒ’ãƒ³ãƒˆè¡ŒãEã‚¯ãƒ©ã‚¹ã«å¯¾å¿œCSSãŒæœªå®Ÿè£E§ã€è¡¨ç¤ºãŒç´ ã®ã¾ã¾ã«ãªã‚‹ç®E‰€ãŒæ®‹ã£ã¦ãEŸã€E- `apps/web/src/pages/Arena.tsx` ãEUTF-8 BOM ä»˜ãã«ãªã£ã¦ãŠã‚Šã€å·®åˆEƒã‚¤ã‚ºã¨ã‚¨ãƒ³ã‚³ãƒ¼ãƒE‚£ãƒ³ã‚°ä¸çµ±ä¸€ã‚’é¿ã‘ã‚‹ãŸã‚æ­£è¦åŒ–ãŒå¿E¦ã ã£ãŸã€E- å¤‰æ›´é‹ç”¨ãƒ«ãƒ¼ãƒ«ã«åˆã‚ã›ã€ä»Šå›ã®å·®åˆE‚’å®Ÿè£Eƒ­ã‚°ã¸æ˜ç¤ºã—ã¦ãŠãå¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/mint-theme/mint-theme.css`
  - `mint-page-guide__*` ã‚¹ã‚¿ã‚¤ãƒ«ç¾¤ã‚’è¿½åŠ EEead/grid/item/title/detailE‰ã€E  - `mint-arena-difficulty__top` / `mint-arena-difficulty__hint` ã‚’è¿½åŠ ã€E  - ãƒ¬ã‚¹ãƒãƒ³ã‚·ãƒ–æ™‚ã® `mint-page-guide__grid` ã‚E`1100px` / `760px` ã§æ®µéšçš„ã«ç¸®é€€ã€E- `apps/web/src/pages/Arena.tsx`
  - é›£æ˜“åº¦é¸æŠãEã‚¿ãƒ³ã« `type="button"` ã‚’è¿½åŠ ã—ã¦ãƒ•ã‚©ãƒ¼ãƒ æ–E„ˆã§ã®èª¤ submit ã‚’äºˆé˜²ã€E  - UTF-8 BOM ã‚’é™¤å»ã—ã€UTF-8 (BOMãªãE ã«çµ±ä¸€ã€E- `docs/99_dev/Nyano_Triad_League_DEV_TODO_v1_ja.md`
  - æœ¬ follow-up ã®å®ŒäºE E›®ã‚’è¿½è¨˜ã€E
### Verify
- `pnpm -C apps/web test`
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web build`
...
### Why
- `apps/web/src/lib/mint_page_guides.ts` ã«ã¯ `events/replay/stream` ã®æ–E¨€å®šç¾©ãŒã‚ã‚‹ä¸€æ–¹ã€å®Ÿéš›ã®ç”»é¢åæ˜ ã¯ `Arena` ã®ã¿ã§ã€å®šç¾©ã¨å®Ÿè£EŒä¸æ•´åˆã ã£ãŸã€E- ç”»é¢é·ç§»æ™‚ãEå­¦ç¿’å°ç·šã‚’æEˆã€MintäºŒæ¬¡ç”»é¢ã®æƒE ±è¨­è¨ˆã‚’çµ±ä¸€ã™ã‚‹å¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/src/pages/Events.tsx`
  - `MintPageGuide` / `MINT_PAGE_GUIDES` ã‚’å°åEã—ã€MintãƒEEãƒæ™‚ã« `MINT_PAGE_GUIDES.events` ã‚’è¡¨ç¤ºã€E- `apps/web/src/pages/Replay.tsx`
  - `MintPageGuide` / `MINT_PAGE_GUIDES` ã‚’å°åEã—ã€`!isStageFocus` æ¡ä»¶ä¸‹ã§ `MINT_PAGE_GUIDES.replay` ã‚’è¡¨ç¤ºã€E  - Stage focus ã® board-first å°ç·šãEç¶­æŒã€E- `apps/web/src/pages/Stream.tsx`
  - `MintPageGuide` / `MINT_PAGE_GUIDES` ã‚’å°åEã—ã€MintãƒEEãƒæ™‚ã« `MINT_PAGE_GUIDES.stream` ã‚’è¡¨ç¤ºã€E
### Verify
- `pnpm -C apps/web lint`
- `pnpm -C apps/web typecheck`
- `pnpm -C apps/web test`
...
### Why
- å…±é€šã‚¬ã‚¤ãƒ‰ãE UI å°ç·šãEåœŸå°ãªã®ã§ã€åEã³ã€Œå®šç¾©ã ã‘ã‚ã‚Šæœªè¡¨ç¤ºã€ã«ãªã‚‹å›å¸°ã‚’é˜²ãå¿E¦ãŒã‚ã£ãŸã€E
### What
- `apps/web/e2e/mint-app-screens-guardrails.spec.ts`
  - `/arena` `/events` `/replay` `/stream` ã§ `.mint-page-guide` å¯è¦–ã‚’è¿½åŠ æ¤œè¨¼ã€E  - æ—¢å­˜ãE 390px åˆ°é”æ€§ãƒ»æ¨ªã‚ªãƒ¼ãƒãEãƒ•ãƒ­ãƒ¼æ¤œè¨¼ã¨åŒæ™‚ã«ãƒã‚§ãƒE‚¯ã€E
### Verify
- `pnpm.cmd -C apps/web e2e -- e2e/mint-app-screens-guardrails.spec.ts`
## 2026-02-17 â€EStage focus overlap fix + UX guardrail commit fallback hardening

### Why
- CI ã§ `e2e/stage-focus.spec.ts` ã®ã€ŒboardAboveDockã€åˆ¤å®šãŒå¤±æ•—ã—ã€`/battle-stage` ã§ç›¤é¢ä¸‹ç«¯ã¨ hand dock ãŒé‡ãªã‚‹ã‚±ãƒ¼ã‚¹ãŒç¢ºèªã•ã‚ŒãŸã€E- åŒã˜ CI å®Ÿè¡Œã§ `ux-guardrails` ã® `Quick commit move` ã‚¯ãƒªãƒE‚¯ãŒã‚¿ã‚¤ãƒŸãƒ³ã‚°ä¾å­˜ã§ flaky ã«ãªã£ã¦ãEŸã€E
### What
- `apps/web/src/mint-theme/mint-theme.css`
  - `.mint-focus-hand-dock--stage` ã® transform ã‚E`translate(-50%, 0)` ã«çµ±ä¸€ã€E  - desktop æ¡ä»¶EEmin-width: 1200px` ã‹ã¤ `min-height: 700px`E‰ã§ `translate(-50%, 10px)` ã‚’é©ç”¨ã—ã€board/dock ã®é‡ãªã‚Šã‚’è§£æ¶ˆã€E- `apps/web/e2e/ux-guardrails.spec.ts`
  - `commitMove` ã§ quick commit ã®ã‚¯ãƒªãƒE‚¯ã«çŸ­ãEtimeout ã¨å¤±æ•—æ™‚ãƒ•ã‚©ãƒ¼ãƒ«ãƒãƒƒã‚¯ã‚’è¿½åŠ ã—ã€æç”»ã‚¿ã‚¤ãƒŸãƒ³ã‚°å·®ã® flaky ã‚’æŠ‘åˆ¶ã€E- `apps/web/src/lib/stage_layout.ts`
  - battle desktop ã® reserve height ã‚E380 â†E400 ã«èª¿æ•´ã€E- `apps/web/src/lib/__tests__/stage_layout.test.ts`
  - ä¸Šè¨˜èª¿æ•´ã«åˆã‚ã›ã¦æœŸå¾E€¤ã‚’æ›´æ–°ã€E
### Verify
- `pnpm -C apps/web typecheck`
...
- `pnpm.cmd -C apps/web e2e` ã¯ãƒ­ãƒ¼ã‚«ãƒ«å®Ÿè¡Œç’°å¢EE `spawn EPERM` ã§å®Œèµ°ä¸å¯Eˆå¯¾è±¡specã¯å€‹åˆ¥å®Ÿè¡Œã§ç¢ºèªï¼‰ã€E## 2026-02-17 - ui=mint parity follow-up: align board/hand/commentary/status flow with Pixi
...
### Why
- Home/Start/Stream contained developer-facing or roadmap-like copy (`ç¾åœ¨ã®ãƒ•ã‚§ãƒ¼ã‚º`, `æ¬¡ã®ãƒã‚¤ãƒ«ã‚¹ãƒˆãEãƒ³`, `å°ç·š`, `æš«å®š`, `DONE/TODO`) visible to end users.
...
- Replay
  ...
  - `apps/web/src/lib/replay_timeline.ts`
    - Localized phase/status labels (`æº–å‚™/åºç›¤/ä¸­ç›¤/çµ‚ç›¤/çµ‚å±€`, `åˆæœŸç›¤é¢`).
  - `apps/web/src/lib/replay_highlights.ts`
    - Localized highlight labels (`å¤§é‡åè»¢/é€£éEã‚³ãƒ³ãƒEè­¦å‘Š`).
...
## 2026-02-17 - Stream/HUD æ—¥æœ¬èªã‚³ãƒ”ãEè¿½è£œã¨èª¬æ˜æ–‡ã‚¯ãƒªãƒ¼ãƒ³ã‚¢ãƒEE

### Why
- Stream å‘¨è¾ºã«è‹±èªå¯E‚Šã®è£œåŠ©æ–E¨€ãƒ»ãƒˆãEã‚¹ãƒˆãŒæ®‹ã£ã¦ãŠã‚Šã€æ—¥æœ¬èªãEãƒ¼ã‚¹ã®ä½“é¨“ã«ãƒ ãƒ©ãŒã‚ã£ãŸã€E- `_design/Home` ã«ãƒ¦ãƒ¼ã‚¶ãƒ¼ã«è¦‹ã›ã‚‹ã¹ãã§ãªãE€²æ—ç³»è¡¨ç¾Eˆãƒ•ã‚§ãƒ¼ã‚º/ãƒã‚¤ãƒ«ã‚¹ãƒˆãEãƒ³E‰ãŒæ®‹å­˜ã—ã¦ãEŸã€E
### What
- `apps/web/src/pages/Stream.tsx`
  - çµŒéæ™‚é–“ãƒ©ãƒ™ãƒ«ã‚’æ—¥æœ¬èªåŒ–EEãŸã£ãŸä»Š` / `ç§’å‰` / `åˆE‰`E‰ã€E  - è¦–èEè€E‘ã‘æ¡ˆåEã‚³ãƒ”ãEæ™‚ãƒˆãƒ¼ã‚¹ãƒˆã‚’æ—¥æœ¬èªåŒ–ã€E  - ãƒ©ã‚¤ãƒ–çŠ¶æ…‹ãE `Event` / `Turn` è¡¨ç¤ºã‚E`ã‚¤ãƒ™ãƒ³ãƒE / `ã‚¿ãƒ¼ãƒ³` ã«çµ±ä¸€ã€E- `apps/web/src/components/stream/StreamSharePanel.tsx`
  - è¦–èEè€E¡ˆåE/Nightbot ã‚³ãƒ”ãEå®ŒäºEƒˆãƒ¼ã‚¹ãƒˆã‚’æ—¥æœ¬èªåŒ–ã€E- `apps/web/src/components/stream/WarudoBridgePanel.tsx`
  - `viewer cmd format` è¡¨ç¤ºã‚’æ—¥æœ¬èªãƒ©ãƒ™ãƒ«ã¸å¤‰æ›´ã€E  - ã‚µãƒ³ãƒ—ãƒ«å‡ºåŠ›ãƒ–ãƒ­ãƒE‚¯ã®ã‚¤ãƒ³ãƒEƒ³ãƒˆãšã‚Œã‚’æ•´å½¢ã€E- `apps/web/src/components/StreamOperationsHUD.tsx`
  - ãƒ˜ãƒƒãƒ€ãƒ¼æ•´å½¢ã‚’ä¿®æ­£ã€E  - `VOTE OPEN` / `Vote Status` ãªã©ã‚’æ—¥æœ¬èªãƒ©ãƒ™ãƒ«ã¸èª¿æ•´ã€E  - `Last Error` / `Health` / `Ops Log` ã‚’æ—¥æœ¬èªåŒ–ã€E  - ç›¸å¯¾æ™‚åˆ»è¡¨ç¤ºã‚’æ—¥æœ¬èªåŒ–ã€E- `apps/web/src/pages/_design/Home.tsx`
  - ã€Œç¾åœ¨ã®ãƒ•ã‚§ãƒ¼ã‚ºã€ã€Œæ¬¡ã®ãƒã‚¤ãƒ«ã‚¹ãƒˆãEãƒ³ã€ã‚’ãƒ¦ãƒ¼ã‚¶ãƒ¼å‘ã‘æ–E¨€ã¸ç½®æ›ã€E  - é€²æ—èª¬æ˜èª¿ã®ãƒE‚­ã‚¹ãƒˆã‚’ä¸€èˆ¬çšEªåˆ©ç”¨å°ç·šèª¬æ˜ã¸æ›´æ–°ã€E
### Verify
- pnpm -C apps/web test OK
- pnpm -C apps/web typecheck OK
- pnpm -C apps/web build OK
...
## 2026-02-17 - Overlay æ—¥æœ¬èªãEãƒ¼ã‚¹åŒ–ï¼E2Eäº’æ›ã‚­ãƒ¼ãƒ¯ãƒ¼ãƒ‰ç¶­æŒE¼E
### Why
- Overlay ã¯é…ä¿¡è¡¨ç¤ºã§ã®éœ²å‡ºãŒå¤šã„ä¸€æ–¹ã€è£œåŠ©æ–E¨€ãŒè‹±èªä¸­å¿E§æ—¥æœ¬èªãƒˆãƒ¼ãƒ³ã¨ä¸æ•´åˆã ã£ãŸã€E- ãŸã ãEE2E ãE`Now Playing` / `Chat voting` / `No signal yet` / `OPEN` / `remaining` ã«ä¾å­˜ã—ã¦ãE‚‹ãŸã‚äº’æ›ç¶­æŒãŒå¿E¦ã ã£ãŸã€E
### What
- `apps/web/src/pages/Overlay.tsx`
  - æ™‚åˆ»è¡¨è¨˜ã‚’æ—¥æœ¬èªåŒ–EEãŸã£ãŸä»E/ ç§’å‰ / åˆE‰ / æ™‚é–“å‰`E‰ã€E  - ã‚¹ãƒEEã‚¿ã‚¹è¦ç´E‚’æ—¥æœ¬èªåŒ–Eˆå‹è€Eã‚¿ãƒ¼ãƒ³/ã‚¿ã‚¤ãƒ«/å¾E©Ÿä¸­E‰ã€E  - ä¸Šéƒ¨ã‚³ãƒ³ãƒˆãƒ­ãƒ¼ãƒ«ãƒ»è­¦å‘ŠãEç›´å‰æ‰‹ãƒ»æŠ•ç¥¨ãƒ»ã‚¨ãƒ©ãƒ¼ãƒ»ãƒ˜ãƒ«ãƒ—æ–‡è¨€ã‚’æ—¥æœ¬èªãEãƒ¼ã‚¹ã¸èª¿æ•´ã€E  - E2Eä¾å­˜èªãEä½µè¨˜ã—ã¦ç¶­æŒE¼ˆä¾E `å¯¾æˆ¦ä¸­ (Now Playing)`, `æŠ•ç¥¨çŠ¶æ³E(Chat voting)`, `ä¿¡å·å¾E¡ (No signal yet)`, `æ®‹ã‚Š ...s remaining`E‰ã€E  - strictAllowed ã¾ã‚ã‚Šã®è£œåŠ©æ–E¨€ã‚’æ—¥æœ¬èªåŒ–Eˆåˆæ³•æ‰‹/WMå€™è£Eãƒ›ã‚¹ãƒˆå¾E©Ÿä¸­E‰ã€E
### Verify
- pnpm -C apps/web test OK
- pnpm -C apps/web typecheck OK
- pnpm -C apps/web build OK
- pnpm.cmd -C apps/web e2e:ux OK (15 passed)
- `e2e/cross-tab-overlay.spec.ts` / `e2e/smoke.spec.ts` ã®å€‹åˆ¥å®Ÿè¡ŒãEç’°å¢E”±æ¥ `spawn EPERM` ã§æœªå®ŒäºE¼ˆã‚³ãƒãƒ³ãƒ‰å®Ÿè¡ŒèEä½“ãEè©¦è¡Œæ¸ˆã¿E‰ã€E
- `apps/web/src/App.tsx`
  - ãƒ˜ãƒƒãƒ€ãƒ¼ã‚°ãƒ«ãƒ¼ãƒ—è¦‹åEã—ï¼Elay/WatchE‰ã‚’æ—¥æœ¬èªåŒ–ã€E  - Mint/é€šå¸¸ãƒ•ãƒƒã‚¿ãƒ¼ãƒªãƒ³ã‚¯æ–E¨€ã‚’æ—¥æœ¬èªãEãƒ¼ã‚¹ã¸çµ±ä¸€ã€E  - ãƒ•ãƒƒã‚¿ãƒ¼ã‚¿ã‚°ãƒ©ã‚¤ãƒ³ã‚E`æ±ºå®šè«EÂ· ã‚³ãƒŸãƒ¥ãƒ‹ãƒ†ã‚£ä¸»å°` ã«æ›´æ–°ã€E

## 2026-02-17 - Home/Playground “ú–{Œêƒx[ƒX’Ç•âiE2EŒİŠ·Œê‚ğ•Ûj

### Why
- Home ‚Ì Tools/metrics —Ìˆæ‚Æ Playground ‚É‰pŒê—DˆÊ‚Ì•¶Œ¾‚ªc‚Á‚Ä‚¨‚èA“ú–{Œêƒx[ƒX‘ÌŒ±‚Éƒ€ƒ‰‚ª‚ ‚Á‚½B
- ‚½‚¾‚µ Home/Replay ‚Ì E2E ‚Í‰pŒêŒê‹å‚ğ selector ‚Æ‚µ‚ÄQÆ‚·‚é‚½‚ßAŠ®‘S’uŠ·‚Å‚Í‚È‚­ŒİŠ·•¹‹L‚ª•K—v‚¾‚Á‚½B

### What
- apps/web/src/pages/Playground.tsx
  - Œ©o‚µ/‘€ì•¶Œ¾/ó‘Ô•\¦‚ğ“ú–{Œêƒx[ƒX‰»i—á: ƒxƒNƒ^ƒZƒbƒgAƒP[ƒXAƒ^[ƒ“ƒƒOAƒfƒbƒLŠm”FAŒ»İ‚ÌŸÒjB
  - Nyano Lab ‚È‚ÇŠù‘¶E2E‚ªQÆ‚·‚éŒê‚Í•Û‚µ‚Â‚ÂA“ú–{Œêà–¾‚Ö“ˆêB
  - ƒRƒs[‘€ìƒg[ƒXƒg‚ğ“ú–{Œê‰»B
- apps/web/src/pages/Home.tsx
  - Tools/SettingsEŠeƒ{ƒ^ƒ“EƒƒgƒŠƒNƒXƒ‰ƒxƒ‹‚ğ“ú–{Œê—Dæ‚Ö•ÏXB
  - E2EˆË‘¶Œê‚Í•¹‹L‚Å•Ûi—á: Tools / Settings, Copy Snapshot, Reset MetricsjB
  - ƒXƒiƒbƒvƒVƒ‡ƒbƒgŠÖ˜Aƒg[ƒXƒg/‹óó‘Ô•¶Œ¾‚ğ“ú–{Œê‰»B
...
### What
- Decks: İ’èƒ^ƒu–¼‚ğ“ú–{Œê‰»‚µASave Deck ‚ğwƒfƒbƒL•Û‘¶ (Save Deck)x‚Ö“ˆêB•Û‘¶Ï‚İƒfƒbƒL‚ÌƒRƒs[•¶Œ¾‚ğwJSON‚ğƒRƒs[x‚Ö•ÏXBQuick Play•\‹L‚ğwƒNƒCƒbƒN‘Îíx‚Ö’²®B
- Events: æ‚è‚İ¸”sŠù’èƒƒbƒZ[ƒWA‘«ÕƒƒOiMy PawprintsjAƒ[ƒJƒ‹—š—ğ‘€ìAŸ”s•\¦AOpen/Copy/Remove ‚ğ“ú–{Œê‰»B
- Replay: •â••¶Œ¾i–¢“ÇAŒ©‚Ç‚±‚ëˆÚ“®ANyanoƒfƒbƒL tokenIdsAclassic swap/open •â•Aƒc[ƒ‹ƒ`ƒbƒvj‚ğ“ú–{Œê‰»B
...
- Match: E2EˆË‘¶‚Ì•¶Œ¾iGuest Quick PlayACommit moveŒnALoad replayŒnj‚ÍˆÛB
- Decks: Deck Builder/My Deck/Set as A/Edit/Delete/Save Deck ‚Ì‰pŒêƒg[ƒNƒ“‚ğŒİŠ·•¹‹L‚µ‚Â‚Â“ú–{Œê—Dæ‚Ö’²®B
- E2E: pps/web/e2e/guest-game.spec.ts ‚Ì Commit Move ŒÅ’èƒAƒT[ƒVƒ‡ƒ“‚ğŒ»sUIŒİŠ·iQuick commit / Commit move / focus hand dockj‚ÖXVB
...
- apps/web/e2e/mint-app-screens-guardrails.spec.ts
  - Added Decks mint browser interaction guardrail:
    - hand combobox selection reflects in chips (`èD: ƒp[`)
    - query input reflects in chips (`ŒŸõ: 1`)
...
### What
- apps/web/src/pages/Start.tsx
  - Updated done state copy to Japanese-first: `€”õ‚ª‚Å‚«‚Ü‚µ‚½BƒfƒbƒL / ƒAƒŠ[ƒi ‚Öi‚ß‚Ü‚·B`
  - Updated back link label: `ƒz[ƒ€‚Ö–ß‚é`.
- apps/web/src/pages/Replay.tsx
  - Replaced ambiguous `Š®—¹ | step ...` with clearer playback wording: `Ä¶ˆÊ’u | ... è`.
...
- apps/web/src/components/PlayerSidePanelMint.tsx
  - Changed remaining-card visible label to `c‚èƒJ[ƒh`.
  - Changed aria-label to Japanese-first wording: `Player A/B ‚Ìc‚èƒJ[ƒh N`.
...
## 2026-02-18 - Home“±ü2ƒXƒeƒbƒv‰» + stage-focusˆÀ’è‰»

### What
- apps/web/src/pages/Home.tsx
  - ƒz[ƒ€‚ÌƒIƒ“ƒ{[ƒfƒBƒ“ƒO‚ğu2ƒXƒeƒbƒvv“±ü‚Ö®—‚µA3ƒXƒeƒbƒv–ÚƒJ[ƒh‚ğíœB
  - ƒ†[ƒU[Œü‚¯‚Å‚È‚¢i’»Š®—¹•\¦iŠ®—¹ó‘ÔƒƒbƒZ[ƒWj‚ğíœ‚µ‚ÄAˆÄ“à•¶‚ğŠÈŒ‰‰»B
- apps/web/src/mint-theme/mint-theme.css
  - stage focus + hand dock •¹—p‚Ì board Å‘å•‚ğ”÷’²®‚µAboard/dock ‚Ìd‚È‚è‚ğ—}§B
- apps/web/e2e/stage-focus.spec.ts
  - replay load/error/retry ‚Ì•¶Œ¾‚ğ“ú‰p•¹‹L‚Ö’Ç]i`Load replay` / `Ä¶‚ğ“Ç‚İ‚Ş` “™jB
  - ©“®“Ç’†i“Ç‚İ‚İ’†... disabledj‚Å‚à¸”s‰ñ•œƒeƒXƒg‚ªˆÀ’è‚·‚é‚æ‚¤‚É•ªŠò‚ğ‹­‰»B
- apps/web/e2e/home.spec.ts
  - ƒz[ƒ€‚ª2ƒXƒeƒbƒv\¬‚É‚È‚Á‚½‚±‚Æ‚É‡‚í‚¹A•s—v‚È3ƒXƒeƒbƒvŠú‘Ò’l‚ğíœB
...
### What
- apps/web/src/pages/Start.tsx
  - 1•ªƒXƒ^[ƒg“±ü‚ğ2ƒXƒeƒbƒv\¬‚É“ˆêi3ƒXƒeƒbƒv–ÚƒJ[ƒh‚ğ“P‹jB
- apps/web/src/pages/Match.tsx
  - ƒtƒH[ƒJƒX‘€ì/ó‘ÔƒtƒB[ƒhƒoƒbƒN‚ğ“ú–{Œêƒx[ƒX‰»i‰pŒê‚ÍŠ‡ŒÊ•¹‹LjB
  - `Hide/Show Controls` ‚Æ `Hide/Show HUD` ‚Í•\¦•¶Œ¾‚ğ“ú–{Œê‰»‚µ‚Â‚ÂAaria-label ‚ğ‰pŒêŒİŠ·‚ÅˆÛB
  - ‹¤—LURL–¢€”õƒGƒ‰[‚È‚Ç‚ğ“ú–{Œê‰»B
- apps/web/src/pages/Replay.tsx
  - ƒtƒH[ƒJƒX‘€ìƒtƒB[ƒhƒoƒbƒNicontrols/setup/highlight/startj‚ğ“ú–{Œêƒx[ƒX‰»B
- apps/web/src/pages/Stream.tsx
  - Quick nav ‚ÆƒXƒeƒbƒvŒ©o‚µiStep 1/3/4j‚ğ“ú–{Œêƒx[ƒX‰»B
  - BroadcastŒnà–¾/ƒ{ƒ^ƒ“•¶Œ¾‚ğ“ú–{Œêƒx[ƒX‚É®—B
- apps/web/e2e/stage-focus.spec.ts
  - recoveryƒP[ƒX‚ğ share param decode failure (`t=invalid`) ‚ÉŠñ‚¹‚ÄŒˆ’è˜_“I‚ÉˆÀ’è‰»B
...
### What
- apps/web/src/pages/Match.tsx
  - Changed replay-unavailable toast title to Japanese-first (`ƒŠƒvƒŒƒC (Replay)`).
  - Replaced ambiguous stage-focus default feedback from `€”õŠ®—¹` to `‘€ì‚ğ‘I‘ğ‚µ‚Ä‚­‚¾‚³‚¢`.
  - Updated visible commit button text to Japanese-first (`Šm’è (Commit)`) while keeping English aria-labels for E2E compatibility.
- apps/web/src/pages/Replay.tsx
  - Changed initial empty-state error copy to Japanese-first with English parenthetical.
  - Changed overlay-related toast titles/messages to Japanese-first (`ƒI[ƒo[ƒŒƒC (Overlay)`).
  - Replaced ambiguous stage-focus default feedback from `€”õŠ®—¹` to `‘€ì‚ğ‘I‘ğ‚µ‚Ä‚­‚¾‚³‚¢`.
  - Updated stream tools wording to Japanese-first (`ƒI[ƒo[ƒŒƒC...`) with English term parenthetical.
...
### What
- apps/web/src/components/MatchDrawerMint.tsx
  - Drawer title changed to Japanese (`Ú×`) and toggle glyph changed from text to symbol (`ß`).
...
- apps/web/src/components/match/MatchSetupPanelMint.tsx
  ...
  - Updated section labels to: ‘Îíİ’è / Œ»İ‚Ìİ’è / Šî–{İ’è / ’Ç‰Áİ’è / Ú×ƒpƒ‰ƒ[ƒ^.
...
- apps/web/src/pages/Home.tsx
  - Replaced toast suffix wording from Š®—¹ to À{Ï‚İ.
...
- apps/web/src/pages/Stream.tsx
  - Simplified quick-action labels to JP-first (`‘Îí`, `ƒI[ƒo[ƒŒƒC`, `ƒŠƒvƒŒƒC`, `ƒCƒxƒ“ƒg`).
- apps/web/src/pages/Home.tsx
...
### What
- apps/web/src/pages/Replay.tsx
  - Localized additional visible copy to JP-first (`‘Î‹ÇƒƒOJSON`, `‘ÎíID`, `Œ‹‰Ê`, save notifications, overlay URL copy labels).
  - Kept E2E-sensitive phrases where required (`Replay from transcript`, `Replay controls are hidden for board focus.`).
  - Restored `URL fallbacki...j` wording for ruleset fallback label to preserve guardrail compatibility.
...
- `apps/web/src/pages/Stream.tsx`
  ...
  - Localized stream summary labels (`ã‚ªãƒ¼ãƒãƒ¼ãƒ¬ã‚¤`, `å¯¾æˆ¦ãƒã‚¹`) and link labels (`ã‚¤ãƒ™ãƒ³ãƒˆ`, `å¯¾æˆ¦`, `ãƒªãƒ—ãƒ¬ã‚¤`, `ã‚ªãƒ¼ãƒãƒ¼ãƒ¬ã‚¤`).
...
- `apps/web/src/pages/Overlay.tsx`
  ...
  - Localized vote status labels to `å—ä»˜ä¸­` / `å—ä»˜çµ‚äº†` while preserving compatibility via `aria-label` (`OPEN`/`CLOSED`).
...
- `apps/web/src/pages/Replay.tsx`
  - Replaced mixed EN user-facing labels with JP-first wording:
    - `ãƒªãƒ—ãƒ¬ã‚¤èª­è¾¼ï¼ˆReplay from transcriptï¼‰` -> `ãƒªãƒ—ãƒ¬ã‚¤èª­è¾¼`
    - `ãƒªãƒ—ãƒ¬ã‚¤ã‚’èª­ã¿è¾¼ã‚€ (Load replay)` -> `ãƒªãƒ—ãƒ¬ã‚¤ã‚’èª­ã¿è¾¼ã‚€`
    - `ã‚¨ãƒ©ãƒ¼ / Error` -> `ã‚¨ãƒ©ãƒ¼`
    - `å†èª­è¾¼ã™ã‚‹ (Retry load)` -> `å†èª­è¾¼ã™ã‚‹`
    - `å…±æœ‰ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã‚’ã‚¯ãƒªã‚¢ (Clear share params)` -> `å…±æœ‰ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã‚’ã‚¯ãƒªã‚¢`
    - Removed EN parentheticals from focus guard/transport hidden messages.
  - Updated stream-assist copy to JP-first (`step` -> `æ‰‹é †`, placeholder `Playground` -> `ãƒ—ãƒ¬ã‚¤ã‚°ãƒ©ã‚¦ãƒ³ãƒ‰`).
  - Updated copy labels used in toasts (`transcript`/`result` -> `å¯¾å±€ãƒ­ã‚°JSON`/`çµæœJSON`).
- E2E selector compatibility updates:
...
- `apps/web/src/pages/Replay.tsx`
  - Removed remaining mixed-English focus wording in visible controls/feedback:
    - `æ“ä½œã‚’éš ã™ (Hide controls)` / `æ“ä½œã‚’è¡¨ç¤º (Show controls)` -> `æ“ä½œã‚’éš ã™` / `æ“ä½œã‚’è¡¨ç¤º`
    - `æ“ä½œã‚’éš ã—ã¾ã—ãŸ (Controls hidden)` / `è¨­å®šã‚’è¡¨ç¤ºã—ã¾ã—ãŸ (Setup shown)` -> JP-only feedback
    - highlight status text -> JP-only (`è¦‹ã©ã“ã‚ Nä»¶`)
    - `å…ˆé ­ã¸ç§»å‹•ã—ã¾ã—ãŸ (Jumped to start)` -> `å…ˆé ­ã¸ç§»å‹•ã—ã¾ã—ãŸ`
- `apps/web/src/components/match/MatchSetupPanelMint.tsx`
  - Reworded advanced options from dev-centric terms to user-facing JP:
    - `Layer4 chain cap` -> `é€£é–ä¸Šé™`
    - invalid `ccap` message -> JP-first explanatory wording
    - `Layer4 å®Ÿé¨“è¨­å®š...` -> `è¿½åŠ è¨­å®š...`
    - `rulesetId` label -> `ãƒ«ãƒ¼ãƒ«ID`
- E2E compatibility updates
  - `apps/web/e2e/stage-focus.spec.ts`
...
### What
- `apps/web/src/pages/Home.tsx`
  - Main menu titles changed to JP-first (`ã‚¢ãƒªãƒ¼ãƒŠ`, `ãƒ‡ãƒƒã‚­`, `ãƒªãƒ—ãƒ¬ã‚¤`, `é…ä¿¡`).
- `apps/web/src/pages/Events.tsx`
  - Quick action labels changed from mixed `æ—¥æœ¬èª (English)` to JP-first labels.
- `apps/web/src/pages/Decks.tsx`
  - Mint tab labels changed to JP-first (`ãƒ‡ãƒƒã‚­`, `ã‚¢ãƒªãƒ¼ãƒŠ`, `ã‚¤ãƒ™ãƒ³ãƒˆ`, `ãƒªãƒ—ãƒ¬ã‚¤`, `é…ä¿¡`).
...
### What
- `apps/web/src/components/CardBrowserMint.tsx`
  - Added quick hand filter pills (`ã™ã¹ã¦/ã‚°ãƒ¼/ãƒãƒ§ã‚­/ãƒ‘ãƒ¼`) using `MintPressable`.
...
- `apps/web/e2e/mint-app-screens-guardrails.spec.ts`
  - Updated Decks filter interaction to click quick pill (`ãƒ‘ãƒ¼`) and assert chip update.
...
- `apps/web/src/components/CardBrowserMint.tsx`
  ...
  - Added unified reset action (`æ¡ä»¶ãƒªã‚»ãƒƒãƒˆ`) for hand filter, edge filter, and search query.
...
- `apps/web/src/pages/Replay.tsx`
  ...
  - Localized replay focus VFX labels (`©“®/ƒIƒt/’á/’†/‚`) and stage feedback text.
  - Localized compare panel labels (`ƒGƒ“ƒWƒ“ v1` / `ƒGƒ“ƒWƒ“ v2`) to reduce English-heavy UI.
...
- `apps/web/src/pages/Replay.tsx`
  ...
  - Localized detail panel labels: `rulesetId` -> `ƒ‹[ƒ‹ƒZƒbƒgIDirulesetIdj`, `classic “ü‘Ö/ŒöŠJ` -> `ƒNƒ‰ƒVƒbƒN“ü‘Ö/ŒöŠJ`.
...
### What
- `apps/web/e2e/smoke.spec.ts`
  - Relaxed route smoke assertions to accept both English and Japanese page labels (`Arena/ƒAƒŠ[ƒi`, `Ruleset Registry/ƒ‹[ƒ‹ƒZƒbƒgˆê——`, `Events/ƒCƒxƒ“ƒg`).
- `apps/web/e2e/stage-focus.spec.ts`
  - Updated replay-load-failure assertion to match current error copy (`ƒGƒ‰[:`) as well as existing `Error:`.
...
- `apps/web/src/pages/Decks.tsx`
  ...
  - Added density hint labels (`i‚è‚İ: ‹­‚ß/•W€/L‚ß`) based on active ratio to support filter suitability judgment.
