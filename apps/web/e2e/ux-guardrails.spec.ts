...
async function commitMove(page: Page, cardSlot: number, cell: number): Promise<void> {
  ...
  const quickCommitButton = page.locator('.mint-match-quick-commit button[aria-label="手を確定"]').first();
  ...
  const dockCommitButton = page.locator('.mint-focus-hand-dock button[aria-label="手を確定"]').first();
  ...
  const currentHand = page.getByRole("listbox", { name: /プレイヤー[AB]の手札/ }).first();
  const hasHandListbox = (await currentHand.count()) > 0 && await currentHand.isVisible().catch(() => false);
  if (hasHandListbox) {
    const option = currentHand.getByRole("option", { name: new RegExp(`^カード${cardSlot}（`) }).first();
    if ((await option.count()) > 0) {
      await option.click({ force: true });
    }
  }
  const commitButton = page.locator('button[aria-label="手を確定"]:visible').first();
