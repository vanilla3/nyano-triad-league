...
test.describe("stage routes", () => {
  ...
  test("/battle-stage canonicalizes params and renders focus hand dock", async ({ page }) => {
    ...
    await expect(page.getByText("手札ドック")).toBeVisible({ timeout: 10_000 });
    await expect(page.locator('.mint-focus-hand-dock button[aria-label="手を確定"]')).toBeVisible({ timeout: 10_000 });
  });
  ...
  test("/battle-stage keeps dock commit action visible on desktop", async ({ page }) => {
    ...
    const dockCommitVisible = page.locator('.mint-focus-hand-dock button[aria-label="手を確定"]:visible').first();
    await expect(dockCommitVisible).toBeVisible({ timeout: 10_000 });
    await expect(page.locator('.mint-focus-hand-dock select[aria-label="警戒マークの配置先"]')).toBeVisible({ timeout: 10_000 });
    await expect(page.getByLabel("Battle focus toolbar hint")).toHaveCount(0);

    const retryPixiButton = page.getByLabel("Pixi レンダラを再試行");
  ...
  test("/battle-stage supports stage keyboard shortcuts", async ({ page }) => {
    ...
    // Wait for async match boot (deck load -> hand dock render) so the global
    // stage keyboard listener is attached before dispatching shortcuts.
    await expect(page.getByLabel("Focus hand card 1")).toBeVisible({ timeout: 15_000 });
  ...
  test("/battle-stage falls back to mint board when WebGL is unavailable", async ({ page }) => {
    ...
    await expect(page.getByText(/Pixi レンダラが利用できません。/).first()).toBeVisible({ timeout: 15_000 });
    await expect(page.getByLabel("Pixi レンダラを再試行")).toBeVisible({ timeout: 10_000 });
    await expect(page.locator('.mint-focus-hand-dock button[aria-label="手を確定"]')).toBeVisible({ timeout: 10_000 });
  });
  ...
  test("/replay-stage hides transport controls by default on mobile and allows toggle", async ({ page }) => {
    ...
    await expect(page.getByText(/Replay controls are hidden for board focus\.|board focus 中はリプレイ操作を非表示にしています。/)).toBeVisible({ timeout: 10_000 });
  ...
  test("/battle-stage keeps commit control in viewport on 375px width", async ({ page }) => {
    ...
    const commitButton = page.locator('.mint-focus-hand-dock button[aria-label="手を確定"]');
  ...
  test("/replay-stage keeps recovery controls when replay load fails", async ({ page }) => {
    ...
    const loadReplayButton = page.getByRole("button", { name: /Load replay|読み込む/ });
    if (await loadReplayButton.count()) {
      await expect(loadReplayButton.first()).toBeVisible({ timeout: 10_000 });
      await loadReplayButton.first().click();
    }

    await expect(page.getByText(/Error:|エラー\s*\/\s*Error:/).first()).toBeVisible({ timeout: 30_000 });
    await expect(page.getByRole("button", { name: /Retry load|再試行/ })).toBeVisible({ timeout: 10_000 });
