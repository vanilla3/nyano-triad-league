...
import { MatchGuestSetupPanelMint } from "@/features/match/MatchGuestSetupPanelMint";
...
export function MatchPage() {
  ...
  const theme = React.useMemo(() => resolveAppTheme(searchParams), [searchParams]);
  ...
  // Result overlay is auto-shown once the final move animations settle.
  // If the user dismisses it, do not auto-open again until the match resets.
  const resultOverlayDismissedRef = React.useRef(false);
  ...
  const isGameOver = Boolean(sim.ok && turns.length >= 9);
  ...
  const dismissResultOverlay = React.useCallback(() => {
    resultOverlayDismissedRef.current = true;
    setShowResultOverlay(false);
  }, []);

  // Auto-open the result overlay only after the last-move animations settle.
  // This prevents flicker caused by backdrop-filter + stage VFX overlapping.
  React.useEffect(() => {
    if (!isGameOver) {
      // reset when the match is not complete
      resultOverlayDismissedRef.current = false;
      setShowResultOverlay(false);
      return;
    }
    if (resultOverlayDismissedRef.current) return;
    if (showResultOverlay) return;

    // Wait for the final card flight and flip animations to settle.
    if (cardFlight.isFlying) return;
    if (boardAnim.isAnimating) return;

    const t = window.setTimeout(() => {
      if (!resultOverlayDismissedRef.current) setShowResultOverlay(true);
    }, 120);
    return () => window.clearTimeout(t);
  }, [isGameOver, matchId, boardAnim.isAnimating, cardFlight.isFlying, showResultOverlay]);
  ...
  const commitMove = React.useCallback(() => {
    ...
    if (draftCell === null) {
      setError("置きたいマスを選んでください");
      telemetry.recordInvalidAction();
      return;
    }
    if (draftCardIndex === null) {
      setError("カードを選んでください");
      telemetry.recordInvalidAction();
      return;
    }
  ...
  const [winBurstActive, setWinBurstActive] = React.useState(false);
  const winBurstArmedRef = React.useRef(false);
  const lastBoardImpactAtRef = React.useRef(0);

  // Win burst: one-shot celebratory stage effect at game end.
  // Keep it short so it doesn't fight the result overlay backdrop-filter.
  React.useEffect(() => {
    if (!isGameOver) {
      winBurstArmedRef.current = false;
      setWinBurstActive(false);
      return;
    }
    if (winBurstArmedRef.current) return;
    winBurstArmedRef.current = true;
    if (!sim.ok) return;
    if (sim.full.winner === "draw") return;

    setWinBurstActive(true);
    const t = window.setTimeout(() => setWinBurstActive(false), 720);
    return () => window.clearTimeout(t);
  }, [isGameOver, matchId, sim.ok, sim.ok ? sim.full.winner : null]);

  const stageImpactBurstLevel = React.useMemo<"soft" | "medium" | "hard" | "win" | null>(() => {
    if (winBurstActive) return "win";
  ...
  // When result overlay is open, suspend stage bursts to avoid flicker.
  const stageImpactBurstActive = !showResultOverlay && (stageImpactBurst || boardImpactBurst || winBurstActive);
  ...
  return (
    <div
      ...
      data-result-overlay={showResultOverlay ? "1" : undefined}
    >
      {/* Result Overlay */}
      {gameResult && (
        useMintUi ? (
                ...
                <>
                  ...
                  {sfx ? (
                    <button
                      ...
                      title={sfxMuted ? "効果音: OFF（タップでON）" : "効果音: ON（タップでOFF）"}
                      aria-label={sfxMuted ? "効果音をONにする" : "効果音をOFFにする"}
                    >
                      {sfxMuted ? "SE OFF" : "SE ON"}
                    </button>
                  ) : null}
      ...
      {/* Hero */}
      {!isEngineFocus && !useMintUi && !useEngineRenderer && !isRpg && (<section className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white">
      ...
      {/* Guest quick setup (guest mode) */}
      <MatchGuestSetupPanelMint
        isVisible={!isEngineFocus && isGuestMode}
        defaultOpen={searchParams.get("setup") === "1"}
        theme={theme}
        ...
        disabled={isEvent}
        onSetParam={setParam}
        onSelectRulesetKey={handleRulesetKeyChange}
            ...
            >
              ...
              >
                ...
                {isEngine && engineRendererFailed && (
                  <div className={stageLayoutClasses.engineFallbackBannerClassName}>
                    <span title={engineRendererError ?? undefined}>
                      Pixi レンダラが利用できません。代替の盤面UIを表示します。
                    </span>
                    <button
                      type="button"
                      className="btn btn-sm"
                      onClick={handleRetryEngineRenderer}
                      aria-label="Pixi レンダラを再試行"
                    >
                      再試行
                    </button>
                  </div>
                ...
                <MatchHandInteractionArea
                  ...
                  isVsNyanoAi={isVsNyanoAi}
                  aiAutoPlay={aiAutoPlay}
                  idleGuideHand={idleGuideTarget === "select_card" && draftCardIndex === null}
                  onChangeDraftWarningMarkCell={setDraftWarningMarkCell}
                  onCommitMove={commitMove}
                  onUndoMove={undoMove}
                  onAiMove={doAiMove}
