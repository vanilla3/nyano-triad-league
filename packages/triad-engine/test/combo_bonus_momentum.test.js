...
function mkCardsMap(tokensObj) {
  ...
  for (const [k, v] of Object.entries(tokensObj)) {
    ...
    m.set(tokenId, {
      ...
      jankenHand: v.hand,
      // On-chain subset tieScore uses power (=atk+matk+agi). For vectors we treat it as the deterministic stat sum.
...
test("combo: momentum grants triadPlus(+1) to the next card (core+tactics subset)", () => {
  const vectors = JSON.parse(fs.readFileSync(VECTORS_PATH, "utf8"));
  const c = vectors.cases.find((x) => x.name === "chain_flip_two_tiles");
  assert.ok(c, "missing vector case: chain_flip_two_tiles");

  const t = c.transcript;

  // IMPORTANT: TranscriptV1 is { header, turns } (not a flat object).
  const transcript = {
    header: {
      version: t.version,
      seasonId: t.seasonId,
  ...
  // In this vector, A (turn=4) triggers a 2-flip chain => comboCount=3 => Momentum.
  assert.equal(r.turns[4].flipCount, 2);
  assert.equal(r.turns[4].comboCount, 3);
  assert.equal(r.turns[4].comboEffect, "momentum");

  // The next A placement is turn=6 and should receive triadPlus(+1).
  assert.equal(r.turns[6].appliedBonus.triadPlus, 1);
  assert.equal(r.turns[6].appliedBonus.ignoreWarningMark, false);
});
