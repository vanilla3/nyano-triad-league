...
const VECTORS_PATH = path.resolve(__dirname, "../../../test-vectors/core_tactics_shadow_v2.json");
...
function mkCardsMap(tokensObj) {
  ...
  for (const [k, v] of Object.entries(tokensObj)) {
    ...
    const trait = v.trait ? deriveTraitTypeFromNyanoTraitV1(v.trait) : undefined;
...
test("core+tactics+shadow shared vectors (off-chain engine ↔ on-chain v2 candidate)", () => {
  ...
  for (const c of vectors.cases) {
    ...
    const r = simulateMatchV1(transcript, cards, ONCHAIN_CORE_TACTICS_SHADOW_RULESET_CONFIG_V2);
