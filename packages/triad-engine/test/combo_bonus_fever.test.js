...
test("combo: fever triggers at comboCount=5 (flipCount=4) in core+tactics subset", () => {
  const vectors = JSON.parse(fs.readFileSync(VECTORS_PATH, "utf8"));
  const c = vectors.cases.find((x) => x.name === "fever_flips_four_final_turn");
  assert.ok(c, "missing vector case: fever_flips_four_final_turn");
  ...
  // Final A move flips 4 tiles => comboCount=5 => Fever.
  assert.equal(r.turns[8].flipCount, 4);
  assert.equal(r.turns[8].comboCount, 5);
  assert.equal(r.turns[8].comboEffect, "fever");
});
