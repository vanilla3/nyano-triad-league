...
test("core+tactics (on-chain subset): firstPlayer must be playerA (firstPlayer=1 throws)", () => {
  const movesHex = "001021314252637384";
  const warningMarksHex = "ffffffffffffffffff"; // none
  const earthBoostEdgesHex = "ffffffffffffffffff"; // all NONE
  ...
  const transcript = {
    header: {
      ...
      firstPlayer: 1, // INVALID for on-chain subset
      deadline: 9999999999,
