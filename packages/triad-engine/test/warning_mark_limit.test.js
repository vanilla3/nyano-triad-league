...
function mkUniformCardsMap() {
  ...
  for (let id = 1; id <= 10; id++) {
    ...
    m.set(tokenId, {
      ...
      jankenHand: 0,
      combatStatSum: 3, // deterministic tieScore (unused in edge-tie because we don't power-tiebreak)
    });
  }
  return m;
}

test("tactics: warning mark uses are limited to 3 per player (4th use throws)", () => {
  // A uses warning marks on turns 0/2/4/6 (4 times) -> should fail on the 4th mark.
  const movesHex = "001021314252637384";
  const warningMarksHex = "08ff07ff06ff07ffff"; // A: 8,7,6,7 (4 marks), B: none
  const earthBoostEdgesHex = "ffffffffffffffffff"; // on-chain subset requires NONE
